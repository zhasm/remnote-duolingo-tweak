...
// 無限速度調整、倍速学習、快適な TV シリーズ視聴、ビデオ
// ダウンロード、スクリーンショットなどをサポートします。「ほとんどの Web
// サイトに適用可能」
// @name:ko      오디오 및 비디오 향상 스크립트: 무한 속도 조정, 배속 학습,
// 행복한 TV 시리즈 시청, 비디오 다운로드, 스크린샷 등을 지원합니다. "대부분의
// 웹사이트에 적용 가능"
// @name:ru      Скрипт улучшения звука и видео: поддерживает бесконечную
// регулировку скорости и многое другое.
// @name:de      Skript zur Audio- und Videoverbesserung: Unterstützt stufenlose
...
const originalMethods = {
  ...
  customElementsMethods:
      {define: window.customElements.define, get: window.customElements.get}
};
...
function ready(selector, fn, shadowRoot) {
  ...
  function $ready(selector, fn) {
    ...
    if (!docRoot._MutationListeners || !docRoot._MutationObserver) {
      ...
      docRoot._MutationObserver.observe(
          docRoot, {childList: true, subtree: true});
    }
...
function hackAttachShadow() {
  if (window._hasHackAttachShadow_) return try {
 ...
 * @name         original.js
 * @description
 * 存储部分重要的原生函数，防止被外部污染，此逻辑应尽可能前置，否则存储的将是污染后的函数
 * @version      0.0.1
 * @author       xxxily
...
const mediaCore = (function() {
   ...
   * @param {*} mediaElement -
   *     必选，HTMLMediaElement的具体实例，例如网页上的video标签或new Audio()等
  ...
  function createMediaPlusApi(mediaElement) {
    ...
    const mediaPlusBaseApi = {
      ...
      get(keyName) {
        if (originDescriptors[keyName] && originDescriptors[keyName].get &&
            !originMethods[keyName]) {
          return originDescriptors[keyName].get.apply(mediaElement)
        }
      },
      set(keyName, val) {
        if (originDescriptors[keyName] && originDescriptors[keyName].set &&
            !originMethods[keyName] && typeof val !== 'undefined') {
      ...
      apply(keyName) {
        if (originMethods[keyName] instanceof Function) {
          ...
          args.shift();
          // original.console.log(`[mediaPlusApi][${keyName}]
          // 执行原生apply操作`)
    ...
    /**
     * 扩展api列表。实现'playbackRate', 'volume', 'currentTime', 'play',
     * 'pause'的纯api调用效果，具体可用API如下： mediaPlusApi.lockPlaybackRate()
     ...
     */
    const extApiKeys =
        ['playbackRate', 'volume', 'currentTime', 'play', 'pause'];
      ...
      return
          ...
          element.prototype[methodName] = new original.Proxy(originFunc, {
            apply(target, ctx, args) {
              ...
              // original.console.log(`[mediaElementMethodProxy]
              // 执行代理后的${methodName}函数`)
  ...
  /**
   * 劫持 playbackRate、volume、currentTime
   * 属性，并增加锁定的逻辑，从而实现更强的抗干扰能力
   */
  function hijackPrototypeProperty(element, property) {
    ...
    original.Object.defineProperty.call(Object, element.prototype, property, {
      ...
      get: function() {
        ...
        // original.console.log(`[mediaElementPropertyHijack][${property}][get]`,
        // val)
      ...
      set: function(value) {
        // original.console.log(`[mediaElementPropertyHijack][${property}][set]`,
        // value)
        ...
        if (['playbackRate', 'volume', 'currentTime'].includes(property)) {
          const mediaPlusApi = createMediaPlusApi(this);
          if (mediaPlusApi && mediaPlusApi.isLock(property)) {
            // original.console.log(`[mediaElementPropertyHijack]
            // ${property}已被锁定，无法执行相关操作`)
            return
          }
        }
  ...
  function init(mediaCheckerHandler) {
    ...
    mediaCheckerHandler = mediaCheckerHandler instanceof Function ?
        mediaCheckerHandler :
        function() {};
  ...
  return {
    init, mediaPlus, mediaChecker, originDescriptors, originMethods,
        mediaElementList
  }
})();
...
/**
 * 准确地获取对象的具体类型
 * 参见：https://www.talkingcoder.com/article/6333557442705696719
 ...
 * @param concatArr {boolean} -可选
 *     合并数组，默认遇到数组的时候，直接以另外一个数组替换当前数组，将此设置true则，遇到数组的时候一律合并，而不是直接替换
 * @returns {*|void}
 */
...
function getValByPath$1(obj, path) {
  ...
  for (let i = 0; i < pathArr.length; i++) {
    if (!result) break result = result[pathArr[i]];
  }
  ...
  for (let i = 0; i < pathArr.length; i++) {
    if (!result)
      break
        ...
        return Number.isNaN(val) ? Number.isNaN(result[pathArr[i]]) :
                                   result[pathArr[i]] === val
      }
...
function isOutOfDocument(element) {
  ...
  return (
      top === 0 && right === 0 && bottom === 0 && left === 0 && width === 0 &&
      height === 0)
}
  ...
  if (window.trustedTypes && window.trustedTypes.createPolicy) {
    ...
    if (!policy) {
      policy = window.trustedTypes.createPolicy(
          'default', {createHTML: (string) => string});
    }
 ...
 * @param { HTMLElement } -可选 targetElement
 *     目标元素，如果传入，则会将解析后的节点添加到该元素中
...
function parseHTML(htmlString, targetElement) {
  ...
  if (targetElement && targetElement.appendChild) {
    nodes.forEach(node => {
      ...
      try {
        /* 有些网站出于业务需要会对appendChild进行重写，可能会导致appendChild报错，所以这里需要try
         * catch */
        targetElement.appendChild(targetNode);
      } catch (e) {
        console.error(
            '[parseHTML] appendChild error', e, targetElement, targetNode);
      }
...
/**
 * 将行内样式转换成对象的形式
 * @param {string} inlineStyle -必选，例如： position: relative; opacity: 1;
 *     visibility: hidden; transform: scale(0.1) rotate(180deg);
 * @returns {Object}
 */
...
function parseURL(url) {
  ...
  return {
    source: url, protocol: a.protocol.replace(':', ''), host: a.hostname,
        port: a.port, origin: a.origin, search: a.search, query: a.search,
...
const hasUseKey = {
  keyCodeList: [
    13, 16, 17, 18, 27, 32, 37, 38, 39, 40, 49, 50,  51,
    52, 67, 68, 69, 70, 73, 74, 75, 77, 78, 79, 80,  81,
    82, 83, 84, 85, 87, 88, 89, 90, 97, 98, 99, 100, 220
  ],
  keyList: [
    'enter',      'shift',   'control',   'alt', 'escape', ' ',  'arrowleft',
    'arrowright', 'arrowup', 'arrowdown', '1',   '2',      '3',  '4',
    'c',          'd',       'e',         'f',   'i',      'j',  'k',
    'n',          'o',       'p',         'q',   'r',      's',  't',
    'u',          'w',       'x',         'y',   'z',      '\\', '|'
  ],
...
async function getPageWindow() {
  return new Promise(function(resolve, reject) {
    ...
    const listenEventList =
        ['load', 'mousemove', 'scroll', 'get-page-window-event'];
...
function getPageWindowSync(rawFunction) {
  if (window.unsafeWindow) return window.unsafeWindow
    ...
    try {
      rawFunction = rawFunction || window.__rawFunction__ ||
...
function openInTab(url, opts, referer) {
  ...
  if (window.GM_openInTab) {
    window.GM_openInTab(
        url, opts || {active: true, insert: true, setParent: true});
  } else {
    // 创建新的a标签并模拟点击
...
function isMediaElement(element) {
  return element &&
      (element instanceof HTMLMediaElement || element.HTMLMediaElement ||
       element.HTMLVideoElement || element.HTMLAudioElement)
}
 ...
 * https://cloud.tencent.com/developer/article/1803097
 * (当localStorage不能用时，window.localStorage为null，而不是文中的undefined)
 */
...
function isGlobalStorageUsable() {
  return window.GM_setValue && window.GM_getValue && window.GM_deleteValue &&
...
class ConfigManager {
   ...
   *     -必选，配置路径信息：例如：'enhance.blockSetPlaybackRate'
   * @returns {keyName}
   */
   ...
   * @param {String} keyName -必选
   *     存储到localStorage或globalStorage里的键名，例如：'_h5player_enhance_blockSetPlaybackRate'
   * @returns {confPath}
   */
  getConfPath(keyName = '') {
   ...
   */
  set(confPath, val) {
    if (typeof confPath !== 'string' || typeof val === 'undefined' ||
        val === null) {
      return false
    }
    ...
    if (isLocalStorageUsable()) {
      ...
      if (localConf !== null && localConf !== undefined) {
        try {
          ...
          console.error(
              'configManager parse localStorage error:', key, localConf);
        }
...
// */ const hotkeys = myConfig.get('hotkeys') hotkeys[0].disabled = true
...
const configManager = new ConfigManager({
  ...
  config: {
    ...
    hotkeys: [
      ...
      {desc: '全屏', key: 'enter', command: 'setFullScreen'},
      {
        desc: '切换画中画模式',
        key: 'shift+p',
      ...
      {desc: '切换暂停/播放', key: 'space', command: 'switchPlayStatus'},
      {desc: '减速播放', key: 'x', command: 'setPlaybackRateDown', args: -0.1},
      {desc: '加速播放', key: 'c', command: 'setPlaybackRateUp', args: 0.1},
      {desc: '正常速度播放', key: 'z', command: 'resetPlaybackRate'},
      {
        desc: '设置1x的播放速度',
        key: 'Digit1',
      ...
      {desc: '增加亮度', key: 'E', command: 'setBrightnessUp'},
      {desc: '减少亮度', key: 'W', command: 'setBrightnessDown'},
      {desc: '增加对比度', key: 'T', command: 'setContrastUp'},
      {desc: '减少对比度', key: 'R', command: 'setContrastDown'},
      {desc: '增加饱和度', key: 'U', command: 'setSaturationUp'},
      {desc: '减少饱和度', key: 'Y', command: 'setSaturationDown'},
      {desc: '增加色相', key: 'O', command: 'setHueUp'},
      {desc: '减少色相', key: 'I', command: 'setHueDown'},
      {desc: '模糊增加 1 px', key: 'K', command: 'setBlurUp'},
      {desc: '模糊减少 1 px', key: 'J', command: 'setBlurDown'},
      {desc: '图像复位', key: 'Q', command: 'resetFilterAndTransform'},
      {desc: '画面旋转 90 度', key: 'S', command: 'setRotate'},
      {desc: '播放下一集', key: 'N', command: 'setNextVideo'},
      {
        desc: '插入debugger断点',
...
async function initUiConfigManager() {
  ...
  if (!isUiConfigPage || !isUiConfigMode)
    return
...
let TCC$1 = class TCC {
  constructor(taskConf, doTaskFunc) {
    this.conf = taskConf || {
      ...
      'demo.demo': {
        ...
        // 自定义快捷键的执行方式，如果是组合键，必须是 ctrl-->shift-->alt
        // 这样的顺序，没有可以忽略，键名必须全小写
        shortcuts: {
          /* 注册要执行自定义回调操作的快捷键 */
          register:
              ['ctrl+shift+alt+c', 'ctrl+shift+c', 'ctrl+alt+c', 'ctrl+c', 'c'],
          /* 自定义快捷键的回调操作 */
  ...
  /**
   * 获取域名 ,
   * 目前实现方式不好，需改造，对地区性域名（如com.cn）、三级及以上域名支持不好
   * */
  getDomain() {
  ...
  /**
   * 格式化配置任务
   * @param isAll { boolean } -可选
   *     默认只格式当前域名或host下的配置任务，传入true则将所有域名下的任务配置都进行格式化
   */
  formatTCC(isAll) {
    const t = this;
  ...
  /**
   * 执行当前页面下的相应任务
   * @param taskName {object|string}
   *     -必选，可直接传入任务配置对象，也可用是任务名称的字符串信息，自己去查找是否有任务需要执行
...
class Debug {
  constructor(config = {}) {
    this.config = {
      ...
      styleMap:
          {info: '', log: '', warn: '', error: '', ...config.styleMap || {}}
    };
  ...
  createDebugMethod(name) {
    ...
    return function() {
      ...
      if (trace) {
        if (traceGroup) {
          const arg1Str = typeof arg0 === 'string' ?
              arg0 :
...
//       log: '#95B46A'
//     },
//     style: 'font-size: 22px; font-weight: bold; padding: 2px 4px;
//     border-radius: 2px;', trace: true, traceGroup: true, printTime: true
//   })
...
var debug = Debug$1.create(
    {msg: '[H5player Msg]', trace: false, traceGroup: true, printTime: false});
...
const taskConf = {
  ...
  'demo.demo': {
    ...
    exitWebFullScreen: '.exit-fullscreen-btn',
    autoPlay: '.player-start-btn',
    // pause: ['.player-pause', '.player-pause02'],
    // //多种情况对应不同的选择器时，可使用数组，插件会对选择器进行遍历，知道找到可用的为止
    pause: '.player-pause',
    play: '.player-play',
    ...
    playbackRate: function() {},
    // playbackRate: true, //
    // 当给某个功能设置true时，表示使用网站自身的能力控制视频，而忽略插件的能力
    currentTime: function() {},
    addCurrentTime: '.add-currenttime',
    subtractCurrentTime: '.subtract-currenttime',
    // 自定义快捷键的执行方式，如果是组合键，必须是 ctrl-->shift-->alt
  ...
  'bilibili.com': {
    fullScreen: function() {
      const fullScreen = $q('.bpx-player-ctrl-full') ||
    ...
    webFullScreen: function() {
      ...
      const webFullscreenEnter = $q('.bpx-player-ctrl-web-enter') ||
          $q('.squirtle-pagefullscreen-inactive');
      const webFullscreenLeave = $q('.bpx-player-ctrl-web-leave') ||
          $q('.squirtle-pagefullscreen-active');
      if (oldWebFullscreen || (webFullscreenEnter && webFullscreenLeave)) {
        const webFullscreen = oldWebFullscreen ||
        ...
        /* 取消弹幕框聚焦，干扰了快捷键的操作 */
        setTimeout(function() {
          const danmaku = $q('.bpx-player-dm-input') ||
    ...
    autoPlay: [
      '.bpx-player-ctrl-play', '.squirtle-video-start',
    ...
    shortcuts: {
      ...
      callback: function(h5Player, taskConf, data) {
        ...
        if (!h5Player[keyName]) {
          ...
          if (h5Player[keyName] === h5Player.playbackRate ||
              h5Player[keyName] === true) {
            if (window.sessionStorage.playbackRate &&
              ...
              switch (key) {
                ...
                break case 'x':
                  ...
                  break default: targetSpeed = Number(key);
                  break
              }
    ...
    init: function(h5Player, taskConf) {
      ...
      const setVideoTitle = () => {
        if (wrapEl && wrapEl.querySelector('.video-info-detail')) {
          ...
          const title =
              `${titleText} - ${accountName}`.replace(/[\\/:*?"<>|]/g, '-');
  ...
  'zhihu.com': {
    ...
    play: function(h5Player, taskConf, data) {
      ...
      if (player && player.parentNode && player.parentNode.parentNode) {
        const maskWrap =
            player.parentNode.parentNode.querySelector('div~div:nth-child(3)');
        if (maskWrap) {
  ...
  'twitter.com': {
    init: function(h5Player, taskConf) {
      ...
      const setVideoTitle = () => {
        if (wrapEl && !wrapEl.getAttribute('data-title') &&
    ...
    try {
      ...
      if (taskName === 'shortcuts') {
        ...
        try {
          ...
          debug.error(
              '任务配置中心的自定义函数执行失败：', taskName, taskConf, data,
              e);
          return false
        }
      ...
      debug.error(
          '任务配置中心的自定义任务执行失败：', taskName, taskConf, data, e);
      return false
    }
  })
}
...
function setFakeUA(ua) {
  ...
  // debug.log(customUA, window.location.href, window.navigator.userAgent,
  // document.referrer) if (customUA) {
  //   fakeUA(customUA)
  //   alert(customUA)
  // } else {
  //   alert('ua false')
...
class FullScreen {
  ...
  enterFullScreen() {
    ...
    const enterFn = c.requestFullscreen || c.webkitRequestFullScreen ||
  ...
  enter() {
    const t = this;
    if (t.isFull()) return const container = t.getContainer();
  ...
  exitFullScreen() {
    const d = document;
    const exitFn = d.exitFullscreen || d.webkitExitFullscreen ||
...
var videoCapturer = {
  ...
  capture(video, download, title) {
    ...
    const currentTime = `${Math.floor(video.currentTime / 60)}'${
        (video.currentTime % 60).toFixed(3)}''`;
...
class MouseObserver {
    ...
    if (!t._mouseObserver_[type]) {
      ...
      window.addEventListener(type, (event) => {
        t.observeList.forEach((target) => {
          const isVisibility = target.IntersectionObserverEntry &&
          ...
          if (isVisibility && isReg) {
            ...
            const isNeedTap = offsetX <= bound.width && offsetX >= 0 &&
                offsetY <= bound.height && offsetY >= 0;
            ...
            if (isNeedTap) {
              ...
              listenerList.forEach((listener) => {
                if (listener instanceof Function) {
                  listener.call(t, event, {x: offsetX, y: offsetY}, target);
                }
              });
            }
          }
  ...
  off(target, type, listener) {
    const t = this;
    if (!target || !type || !listener || !t._mouseObserver_ ||
        !t._mouseObserver_[type] || !target.MouseObserverEvent ||
...
class I18n {
  ...
  isMatchCurLang(lang) {
    ...
    return lang.includes(curLang) || lang.includes(curLang2) ||
        lang.includes(curLang3) || lang.includes(curLang4)
  }
 ...
 * @param deep {number} -可选
 *     如果对象层级非常深，则须限定递归的层级，默认最高不能超过3级
 * @returns {{}}
 */
...
const monkeyMsg = {
   ...
   *     -可选，因为会出现莫名奇妙的重复发送情况，为了消除重复发送带来的副作用，所以引入节流限制逻辑，即限制某个时间间隔内只能发送一次，多余的次数自动抛弃掉，默认80ms
   * @returns {Promise<void>}
   */
  ...
  on: (name, fn) => window.GM_addValueChangeListener &&
  ...
  off: (listenerId) => window.GM_removeValueChangeListener &&
...
const crossTabCtl = {
  ...
  updatePictureInPictureInfo() {
    setInterval(function() {
      if (document.pictureInPictureElement) {
        monkeyMsg.send(
            'globalPictureInPictureInfo', {usePictureInPicture: true});
      }
    }, 1000 * 1.5);
  ...
  crossTabKeydownEvent(event) {
    ...
    if (isEditableTarget(target))
      return if (
          t.isNeedSendCrossTabCtlEvent() && isRegisterKey(event) &&
  ...
  bindCrossTabEvent() {
    const t = crossTabCtl;
    if (t._hasBindEvent_)
      return document.removeEventListener('keydown', t.crossTabKeydownEvent);
...
class HookJs {
  ...
  _runHooks(
      parentObj, methodName, originMethod, hookMethod, target, ctx, args,
      classHook, hookPropertiesKeyName) {
    ...
    const execInfo = {result: null, error: null, args: args, type: ''};
     ...
     */
    // if (execInfo.result && execInfo.result.then &&
    // util.isPromise(execInfo.result)) {
  ...
  _proxyMethodcGenerator(
      parentObj, methodName, originMethod, classHook, context, proxyHandler) {
    const t = this;
    ...
    if (useProxy && Proxy) {
      ...
      if (classHook) {
        handler.construct = function(target, args, newTarget) {
          ...
          return t._runHooks(
              parentObj, methodName, originMethod, hookMethod, target, context,
              args, true, t.hookPropertiesKeyName)
        };
      } else {
        handler.apply = function(target, ctx, args) {
          ctx = context || ctx;
          return t._runHooks(
              parentObj, methodName, originMethod, hookMethod, target, ctx,
              args, false, t.hookPropertiesKeyName)
        };
      }
      ...
      hookMethod = function() {
        ...
        return t._runHooks(
            parentObj, methodName, originMethod, hookMethod, originMethod, ctx,
            arguments, classHook, t.hookPropertiesKeyName)
      };
      ...
      keys.forEach(keyName => {
        try {
          ...
          util.debug.log(
              `[proxyMethodcGenerator] hookMethod defineProperty abnormal.  hookMethod:${
                  methodName}, definePropertyName:${keyName}`,
              err);
        }
      });
  ...
  isHook(fn) {
    ...
    return util.isFn(hookMethodProperties.originMethod) &&
  ...
  isAllowHook(parentObj, keyName) {
    /* 有些对象会设置getter，让读取值的时候就抛错，所以需要try catch
     * 判断能否正常读取属性 */
    try {
   ...
   * @param hookMethods {Object|Array|RegExp|string} -必选
   *     被hook函数的函数名或函数名的匹配规则
   * @param fn {Function} -必选 hook之后的回调方法
   * @param type {String} -可选
   *     默认before，指定运行hook函数回调的时机，可选字符串：before、after、replace、error、hangUp
   * @param classHook {Boolean} -可选
   *     默认false，指定是否为针对new（class）操作的hook
   * @param context {Object} -可选 指定运行被hook函数时的上下文对象
   * @param proxyHandler {Object} -可选
   *     仅当用Proxy进行hook时有效，默认使用的是Proxy的apply
   *     handler进行hook，如果你有特殊需求也可以配置自己的handler以实现更复杂的功能
   * 附注：不使用Proxy进行hook，可以获得更高性能，但也意味着通用性更差些，对于要hook
   * HTMLElement.prototype、EventTarget.prototype这些对象里面的非实例的函数往往会失败而导致被hook函数执行出错
   * @returns {boolean}
   */
  hook(parentObj, hookMethods, fn, type, classHook, context, proxyHandler) {
    ...
    if ((!util.isRef(parentObj) && !util.isFn(parentObj)) || !util.isFn(fn) ||
        !hookMethods) {
      return false
    }
    ...
    hookMethods.forEach(methodName => {
      if (!t.isAllowHook(parentObj, methodName)) {
        util.debug.log(`${util.toStr(parentObj)} [${
      ...
      hookMethod = t._proxyMethodcGenerator(
          parentObj, methodName, originMethod, classHook, context,
          proxyHandler);
  ...
  hookClass(parentObj, hookMethods, fn, type, context, proxyHandler) {
    return this.hook(
        parentObj, hookMethods, fn, type, true, context, proxyHandler)
  }
   ...
   * @param hookMethods {Object|Array|RegExp|string} -必选
   *     要取消被hook函数的函数名或函数名的匹配规则
   * @param type {String} -可选
   *     默认before，指定要取消的hook类型，可选字符串：before、after、replace、error、hangUp，如果不指定该选项则取消所有类型下的所有回调
   * @param fn {Function} -必选
   *     取消指定的hook回调函数，如果不指定该选项则取消对应type类型下的所有回调
   * @returns {boolean}
   */
    ...
    hookMethods.forEach(methodName => {
      ...
      if (type) {
        ...
        if (fn) {
          ...
          for (let i = 0; i < hooks.length; i++) {
            if (fn === hooks[i]) {
              ...
              util.debug.log(
                  `[unHook ${hookKeyName} func] ${util.toStr(parentObj)} ${
                      methodName}`,
                  fn);
              break
            }
          ...
          if (Array.isArray(hookMethodProperties[hookKeyName])) {
            hookMethodProperties[hookKeyName] = [];
            util.debug.log(`[unHook all ${hookKeyName}] ${
                util.toStr(parentObj)} ${methodName}`);
          }
        }
      } else {
        /* 彻底还原被hook的函数 */
        if (util.isFn(originMethod)) {
          ...
          //   if (/Hooks$/.test(keyName) &&
          //   Array.isArray(hookMethod[keyName])) {
          //     hookMethod[keyName] = []
          //   }
          // })
          //
  ...
  before(obj, hookMethods, fn, classHook, context, proxyHandler) {
    return this.hook(
        obj, hookMethods, fn, 'before', classHook, context, proxyHandler)
  }

  /* 源函数运行后的hook */
  after(obj, hookMethods, fn, classHook, context, proxyHandler) {
    return this.hook(
        obj, hookMethods, fn, 'after', classHook, context, proxyHandler)
  }

  /* 替换掉要hook的函数，不再运行源函数，换成运行其他逻辑 */
  replace(obj, hookMethods, fn, classHook, context, proxyHandler) {
    return this.hook(
        obj, hookMethods, fn, 'replace', classHook, context, proxyHandler)
  }

  /* 源函数运行出错时的hook */
  error(obj, hookMethods, fn, classHook, context, proxyHandler) {
    return this.hook(
        obj, hookMethods, fn, 'error', classHook, context, proxyHandler)
  }

  /* 底层实现逻辑与replace一样，都是替换掉要hook的函数，不再运行源函数，只不过是为了明确语义，将源函数挂起不再执行，原则上也不再执行其他逻辑，如果要执行其他逻辑请使用replace
   * hook */
  hangUp(obj, hookMethods, fn, classHook, context, proxyHandler) {
    return this.hook(
        obj, hookMethods, fn, 'hangUp', classHook, context, proxyHandler)
  }
}
...
function hackDefineProperCore(target, key, option) {
  if (option && target && target instanceof Element &&
      typeof key === 'string' && key.indexOf('on') >= 0) {
...
function hackDefineProperOnError(
    args, parentObj, methodName, originMethod, execInfo, ctx) {
...
const monkeyMenu = {
  ...
  on(title, fn, accessKey) {
    ...
    if (window.GM_registerMenuCommand) {
      ...
      this.menuIds[menuId] = {title, fn, accessKey};
   ...
   * @param {array|function} menuOpts
   *     菜单配置，如果是函数则会调用该函数获取菜单配置，并且当菜单被点击后会重新创建菜单，实现菜单的动态更新
   */
  build(menuOpts) {
    this.clear();
...
function getHomePage() {
  const homePageLinks =
      ['https://h5player.anzz.top/zh/', 'https://h5player.anzz.top'];
...
function openDocsByPath(path) {
  ...
  const basePath =
      chinese ? 'https://h5player.anzz.top' : 'https://h5player.anzz.top';
...
const globalFunctional = {
  ...
  openCheckVersionPage: {
    ...
    fn: () => {
      const confirm = window.confirm(`${i18n.t('currentVersion')}「${
          version}」\n${i18n.t('checkVersion')}`);
      if (confirm) {
  ...
  toggleExpandedOrCollapsedStateOfMonkeyMenu: {
    title: `${
        configManager.get('enhance.unfoldMenu') ?
            i18n.t('foldMenu') :
            i18n.t('unfoldMenu')} 「${i18n.t('globalSetting')}」`,
    desc: `${
        configManager.get('enhance.unfoldMenu') ?
            i18n.t('foldMenu') :
            i18n.t('unfoldMenu')} 「${i18n.t('globalSetting')}」`,
    fn: () => {
      const confirm = window.confirm(
          configManager.get('enhance.unfoldMenu') ? i18n.t('foldMenu') :
                                                    i18n.t('unfoldMenu'));
      if (confirm) {
        configManager.setGlobalStorage(
            'enhance.unfoldMenu', !configManager.get('enhance.unfoldMenu'));
        window.location.reload();
      }
    }
  },
  ...
  toggleScriptEnableState: {
    title: `${
        (configManager.get('blacklist.domains') || []).includes(location.host) ?
            i18n.t('enableScript') :
            i18n.t('disableScript')} 「${i18n.t('localSetting')}」`,
    desc: `${
        (configManager.get('blacklist.domains') || []).includes(location.host) ?
            i18n.t('enableScript') :
            i18n.t('disableScript')} 「${i18n.t('localSetting')}」`,
    fn: () => {
      ...
      const confirm = window.confirm(
          isInBlacklist ? i18n.t('enableScript') : i18n.t('disableScript'));
      if (confirm) {
        if (isInBlacklist) {
          configManager.setGlobalStorage(
              'blacklist.domains',
              blackDomainList.filter(item => item !== location.host));
        } else {
          configManager.setGlobalStorage(
              'blacklist.domains', blackDomainList.concat(location.host));
        }
  ...
  toggleSetCurrentTimeFunctional: {
    ...
    title: () => `${
        configManager.get('enhance.blockSetCurrentTime') ?
            ...
            i18n.t('blockSetCurrentTime')} 「${i18n.t('localSetting')}」`,
    desc: () => `${
        configManager.get('enhance.blockSetCurrentTime') ?
            i18n.t('unblockSetCurrentTime') :
            i18n.t('blockSetCurrentTime')} 「${i18n.t('localSetting')}」`,
  ...
  toggleSetVolumeFunctional: {
    title: () => `${
        configManager.get('enhance.blockSetVolume') ?
            ...
            i18n.t('blockSetVolume')} 「${i18n.t('localSetting')}」`,
    desc: () => `${
        configManager.get('enhance.blockSetVolume') ?
            i18n.t('unblockSetVolume') :
            i18n.t('blockSetVolume')} 「${i18n.t('localSetting')}」`,
  ...
  toggleSetPlaybackRateFunctional: {
    title: () => `${
        configManager.get('enhance.blockSetPlaybackRate') ?
            ...
            i18n.t('blockSetPlaybackRate')} 「${i18n.t('globalSetting')}」`,
    desc: () => `${
        configManager.get('enhance.blockSetPlaybackRate') ?
            i18n.t('unblockSetPlaybackRate') :
            i18n.t('blockSetPlaybackRate')} 「${i18n.t('globalSetting')}」`,
  ...
  toggleAcousticGainFunctional: {
    title: () => `${
        configManager.get('enhance.allowAcousticGain') ?
            ...
            i18n.t('allowAcousticGain')} 「${i18n.t('globalSetting')}」`,
    desc: () => `${
        configManager.get('enhance.allowAcousticGain') ?
            i18n.t('notAllowAcousticGain') :
            i18n.t('allowAcousticGain')} 「${i18n.t('globalSetting')}」`,
  ...
  toggleCrossOriginControlFunctional: {
    title: () => `${
        configManager.get('enhance.allowCrossOriginControl') ?
            ...
            i18n.t('allowCrossOriginControl')} 「${i18n.t('globalSetting')}」`,
    desc: () => `${
        configManager.get('enhance.allowCrossOriginControl') ?
            i18n.t('notAllowCrossOriginControl') :
            i18n.t('allowCrossOriginControl')} 「${i18n.t('globalSetting')}」`,
  ...
  toggleExperimentFeatures: {
    title: () => `${
        configManager.get('enhance.allowExperimentFeatures') ?
            ...
            i18n.t('allowExperimentFeatures')} 「${i18n.t('globalSetting')}」`,
    desc: () => `${
        configManager.get('enhance.allowExperimentFeatures') ?
            i18n.t('notAllowExperimentFeatures') :
            i18n.t('allowExperimentFeatures')} 「${i18n.t('globalSetting')}」`,
  ...
  toggleDebugMode: {
    title: () => `${
        configManager.getGlobalStorage('debug') ?
            ...
            i18n.t('openDebugMode')} 「${i18n.t('globalSetting')}」`,
    desc: () => `${
        configManager.getGlobalStorage('debug') ?
            i18n.t('closeDebugMode') :
            i18n.t('openDebugMode')} 「${i18n.t('globalSetting')}」`,
    fn: () => {
      const confirm = window.confirm(
          configManager.getGlobalStorage('debug') ? i18n.t('closeDebugMode') :
                                                    i18n.t('openDebugMode'));
      if (confirm) {
        configManager.setGlobalStorage(
            'debug', !configManager.getGlobalStorage('debug'));
        window.location.reload();
      }
    }
  },
  ...
  toggleGUIStatus: {
    title: () => `${
        configManager.getGlobalStorage('ui.enable') === false ?
            i18n.t('enableGUI') :
            i18n.t('disableGUI')} 「${i18n.t('globalSetting')}」`,
    desc: () => `${
        configManager.getGlobalStorage('ui.enable') === false ?
            i18n.t('enableGUI') :
            i18n.t('disableGUI')} 「${i18n.t('globalSetting')}」`,
    fn: () => {
      ...
      if (confirm) {
        configManager.setGlobalStorage(
            'ui.enable', !configManager.getGlobalStorage('ui.enable'));
        window.location.reload();
      }
    }
  },
  ...
  toggleGUIStatusUnderCurrentSite: {
    title: () => `${
        configManager.getLocalStorage('ui.enable') === false ?
            i18n.t('enableGUI') :
            i18n.t('disableGUI')} 「${i18n.t('localSetting')}」`,
    desc: () => `${
        configManager.getLocalStorage('ui.enable') === false ?
            i18n.t('enableGUI') :
            i18n.t('disableGUI')} 「${i18n.t('localSetting')}」`,
    fn: () => {
      ...
      if (confirm) {
        configManager.setLocalStorage(
            'ui.enable', !configManager.getLocalStorage('ui.enable'));
        window.location.reload();
      }
    }
  },
  alwaysShowGraphicalInterface: {
    title: `${i18n.t('toggleStates')}${
        i18n.t('alwaysShowGraphicalInterface')} 「${i18n.t('globalSetting')}」`,
    desc: `${i18n.t('toggleStates')}${
        i18n.t('alwaysShowGraphicalInterface')} 「${i18n.t('globalSetting')}」`,
    fn: () => {
      const alwaysShow = configManager.getGlobalStorage('ui.alwaysShow');
      const confirm = window.confirm(
          alwaysShow === true ? `${i18n.t('disable')}${
                                    i18n.t('alwaysShowGraphicalInterface')} 「${
                                    i18n.t('globalSetting')}」` :
                                `${i18n.t('alwaysShowGraphicalInterface')} 「${
                                    i18n.t('globalSetting')}」`);
      if (confirm) {
  ...
  toggleHotkeysStatus: {
    title: () => `${
        configManager.getGlobalStorage('enableHotkeys') === false ?
            ...
            i18n.t('disableHotkeys')} 「${i18n.t('globalSetting')}」`,
    desc: () => `${
        configManager.getGlobalStorage('enableHotkeys') === false ?
            i18n.t('enableHotkeys') :
            i18n.t('disableHotkeys')} 「${i18n.t('globalSetting')}」`,
    fn: () => {
      ...
      if (confirm) {
        configManager.setGlobalStorage(
            'enableHotkeys', !configManager.getGlobalStorage('enableHotkeys'));
        window.location.reload();
      }
    }
  },
  ...
  toggleHotkeysStatusUnderCurrentSite: {
    title: () => `${
        configManager.getLocalStorage('enableHotkeys') === false ?
            i18n.t('enableHotkeys') :
            i18n.t('disableHotkeys')} 「${i18n.t('localSetting')}」`,
    desc: () => `${
        configManager.getLocalStorage('enableHotkeys') === false ?
            i18n.t('enableHotkeys') :
            i18n.t('disableHotkeys')} 「${i18n.t('localSetting')}」`,
    fn: () => {
      ...
      if (confirm) {
        configManager.setLocalStorage(
            'enableHotkeys', !configManager.getLocalStorage('enableHotkeys'));
        window.location.reload();
      }
    }
  },
  ...
  toggleMouseControl: {
    title: () => `${
        configManager.getGlobalStorage('mouse.enable') === false ?
            i18n.t('mouse.enable') :
            i18n.t('mouse.disable')} 「${i18n.t('globalSetting')}」`,
    desc: () => `${
        configManager.getGlobalStorage('mouse.enable') === false ?
            i18n.t('mouse.enable') :
            i18n.t('mouse.disable')} 「${i18n.t('globalSetting')}」`,
    fn: () => {
      ...
      if (confirm) {
        configManager.setGlobalStorage(
            'mouse.enable', !configManager.getGlobalStorage('mouse.enable'));
        window.location.reload();
      }
    }
  },
  ...
  toggleMouseControlUnderCurrentSite: {
    title: () => `${
        configManager.getLocalStorage('mouse.enable') === false ?
            i18n.t('mouse.enable') :
            i18n.t('mouse.disable')} 「${i18n.t('localSetting')}」`,
    desc: () => `${
        configManager.getLocalStorage('mouse.enable') === false ?
            i18n.t('mouse.enable') :
            i18n.t('mouse.disable')} 「${i18n.t('localSetting')}」`,
    fn: () => {
      ...
      if (confirm) {
        configManager.setLocalStorage(
            'mouse.enable', !configManager.getLocalStorage('mouse.enable'));
        window.location.reload();
      }
    }
  },
  ...
  setMouseLongPressTime: {
    ...
    fn: () => {
      const longPressTime = prompt(
          `${i18n.t('mouse.longPressTime')}「${i18n.t('globalSetting')}」`,
          configManager.getGlobalStorage('mouse.longPressTime') || 600);
      if (longPressTime) {
        configManager.setGlobalStorage(
            'mouse.longPressTime', Number(longPressTime));
        window.location.reload();
      }
    }
  },
  ...
  toggleDownloadControl: {
    title: () => `${
        configManager.getGlobalStorage('download.enable') === false ?
            ...
            i18n.t('mediaDownload.disable')} 「${i18n.t('globalSetting')}」`,
    desc: () => `${
        configManager.getGlobalStorage('download.enable') === false ?
            i18n.t('mediaDownload.enable') :
            i18n.t('mediaDownload.disable')} 「${i18n.t('globalSetting')}」`,
  ...
  toggleDownloadControlUnderCurrentSite: {
    title: () => `${
        configManager.getLocalStorage('download.enable') === false ?
            i18n.t('mediaDownload.enable') :
            i18n.t('mediaDownload.disable')} 「${i18n.t('localSetting')}」`,
    desc: () => `${
        configManager.getLocalStorage('download.enable') === false ?
            i18n.t('mediaDownload.enable') :
            i18n.t('mediaDownload.disable')} 「${i18n.t('localSetting')}」`,
  ...
  setLanguage: {
    ...
    fn: (lang) => {
      const confirm =
          window.confirm(`${i18n.t('languageSettings')}[${lang}] ?`);
      if (confirm) {
...
function registerH5playerMenus(h5player) {
  ...
  if (player && !t._hasRegisterH5playerMenus_) {
    const menus = [
      ...
      {...globalFunctional.toggleDebugMode, disable: foldMenu}
    ];
...
const mediaSource = (function() {
  ...
  function connectMediaSourceWithMediaElement(mediaEl) {
    ...
    mediaSourceMap.forEach(mediaSourceInfo => {
      if (mediaSourceInfo.mediaSource.__objURL__ &&
  ...
  function cleanMediaSourceData() {
    function removeMediaSourceData(mediaSourceInfo) {
      console.log(
          ...
          mediaSourceInfo.mediaUrl || mediaSourceInfo.mediaSource.__objURL__);
    ...
    mediaSourceMap.forEach((mediaSourceInfo) => {
      if (!mediaSourceInfo.mediaElement ||
  ...
  function proxyMediaSourceMethod() {
    ...
    originURLMethods.createObjectURL = originURLMethods.createObjectURL ||
        URL.prototype.constructor.createObjectURL;
    URL.prototype.constructor.createObjectURL =
        new original.Proxy(originURLMethods.createObjectURL, {
          apply(target, ctx, args) {
            const object = args[0];
    ...
    MediaSource.prototype.addSourceBuffer =
        new original.Proxy(originMethods.addSourceBuffer, {
          apply(target, ctx, args) {
            ...
            try {
              // mimeCodecs字符串示例：'video/mp4; codecs="avc1.42E01E,
              // mp4a.40.2"'
              ...
              mediaInfo.codecs =
                  tmpArr[1].trim().replace('codecs=', '').replace(/["']/g, '');
            } catch (e) {
            ...
            sourceBuffer.appendBuffer =
                new original.Proxy(sourceBufferItem.originAppendBuffer, {
                  apply(bufTarget, bufCtx, bufArgs) {
                    ...
                    if (!original.map.get.call(mediaSourceMap, ctx)) {
                      original.map.set.call(
                          mediaSourceMap, ctx, mediaSourceInfo);
                    }
    ...
    MediaSource.prototype.endOfStream =
        new original.Proxy(originMethods.endOfStream, {
          apply(target, ctx, args) {
            /* 标识当前媒体流已加载完成 */
            const mediaSourceInfo = original.map.get.call(mediaSourceMap, ctx);
            if (mediaSourceInfo) {
  ...
  function downloadMediaSource(mediaEl, title) {
    ...
    mediaSourceMap.forEach(mediaSourceInfo => {
      ...
      // original.console.log('[downloadMediaSource][mediaSourceInfo]',
      // mediaSourceInfo)
      ...
      if (!mediaSourceInfo.hasDownload && !mediaSourceInfo.endOfStream) {
        // original.console.log('[downloadMediaSource] 媒体数据还没完全就绪',
        // mediaSourceInfo)

        const confirm =
            original.confirm(i18n.t('mediaDownload.notEndOfStream'));
        if (!confirm) {
          if (mediaSourceInfo.autoDownload) {
            const cancelAutoDownload =
                original.confirm(i18n.t('mediaDownload.cancelAutoDownload'));
            if (cancelAutoDownload) {
              mediaSourceInfo.autoDownload = false;
            }
          } else {
            const autoDownload =
                original.confirm(i18n.t('mediaDownload.autoDownload'));
            if (autoDownload) {
      ...
      mediaSourceInfo.sourceBuffer.forEach(sourceBufferItem => {
        if (!sourceBufferItem.mimeCodecs ||
        ...
        try {
          let mediaTitle = `${
              mediaSourceTitle || sourceBufferItem.mediaInfo.title || title ||
              mediaEl.getAttribute('data-title') || document.title ||
              Date.now()}`;

          if (!mediaSourceTitle && !sourceBufferItem.mediaInfo.title) {
            mediaTitle = original.prompt(
                i18n.t('mediaDownload.confirmTitle'), mediaTitle);
          ...
          /* 自动补充媒体类型和文件后缀 */
          mediaTitle = `${mediaTitle}_${sourceBufferItem.mediaInfo.type}.${
  ...
  return {
    init, hasInit, originMethods, originURLMethods, mediaSourceMap,
        objectURLMap, downloadMediaSource, cleanMediaSourceData,
...
function isModifierKey(key) {
  return [
    'ctrl', 'controlleft', 'controlright', 'shift', 'shiftleft', 'shiftright',
...
const combinationKeysMonitor = (function() {
  ...
  return {
    combinationKeysState, getCombinationKeys, init
  }
})();
...
class HotkeysRunner {
  constructor(hotkeys, win = window) {
    ...
    this.MOD = typeof navigator === 'object' &&
  ...
  run(opts = {}) {
    ...
    this.hotkeys.forEach(hotkeyConf => {
       ...
       */
      if (this.prevPress && hotkeyConf.keyBindings.length > 1 &&
          this.isMatchPrevPress(press)) {
       ...
       */
      if (hotkeyConf.keyBindings.length > 1 &&
      ...
      if (hotkeyConf.when && conditionHandler instanceof Function) {
        const isMatchCondition =
            conditionHandler.apply(target, toArrArgs(hotkeyConf.when));
...
function MediaElementAmplifier(mediaElem) {
  ...
  this._source =
      this._context.createMediaElementSource(this._element = mediaElem);
  this._source.connect(this._gain = this._context.createGain());
  this._gain.connect(this._context.destination);
}
['getContext', 'getSource', 'getGain', 'getElement',
 [
   'getVolume',
...
function mediaDownload(mediaEl, title, downloadType) {
  ...
  if (mediaEl && mediaUrl && !mediaUrl.startsWith('blob:')) {
    ...
    let mediaTitle = `${
        title || mediaEl.getAttribute('data-title') || document.title ||
        Date.now()}_${mediaInfo.type}.${mediaInfo.format}`;
    ...
    if (downloadType === 'blob' || mediaEl.duration < 60 * 5) {
      ...
      mediaTitle =
          original.prompt(i18n.t('mediaDownload.confirmTitle'), mediaTitle);
      if (!mediaTitle) {
        return false
      }
      ...
      if (mediaUrl.startsWith('http://') &&
        ...
        /* 在https里fetch http资源会导致 block:mixed-content
         * 错误，所以尝试将地址统一成https开头 */
...
const device = {
  ...
  isTablet: () => {return /iPad/i.test(navigator.userAgent)},
  ...
  isSafari: () => {return /Safari/i.test(navigator.userAgent)},
  isEdge: () => {
...
const remoteHelper = {
  init() {
    ...
    const contactRemoteHelperSuccessTime =
        configManager.getGlobalStorage('contactRemoteHelperSuccessTime');
    let lastContactRemoteHelperTime =
        configManager.getGlobalStorage('lastContactRemoteHelperTime');
    ...
    const syncInterval =
        ...
        1000 * 60 * 60 * 12;
    if (contactRemoteHelperSuccessTime &&
  ...
  establishRemoteConnection() {
    const lastSucTime =
        configManager.getGlobalStorage('contactRemoteHelperSuccessTime') || '0';
    const timeStr = new Date().toISOString().split('T')[0].replace(/-/g, '') +
        new Date().getHours() + '' + new Date().getMinutes();
    const iframe = document.createElement('iframe');
    iframe.src =
        `${remoteHelperUrl}?t=${timeStr}&v=${version}&lst=${lastSucTime}`;
  ...
  async remoteHandler() {
    if (!location.href.startsWith(remoteHelperUrl) ||
    ...
    function syncRemoteData(pageWindow) {
      if (pageWindow.recommendList) {
        configManager.setGlobalStorage(
            'recommendList', pageWindow.recommendList);
      }

      /* 待增加版本对比判断逻辑 */
      if (pageWindow.remoteVersion) {
        configManager.setGlobalStorage(
            'remoteVersion', pageWindow.remoteVersion);
      }

      if (pageWindow.remoteHelperSyncInterval) {
        configManager.setGlobalStorage(
            'remoteHelperSyncInterval', pageWindow.remoteHelperSyncInterval);
      }

      configManager.setGlobalStorage(
          'contactRemoteHelperSuccessTime', Date.now());
    }
...
const h5playerUI = function(window) {
  var h5playerUI = (function() {
    ...
    const
        r$6 = t => new n$5('string' == typeof t ? t : t + '', void 0, s$3),
        i$3 =
            (t, ...e) => {
              const o = 1 === t.length ?
                  t[0] :
            ...
            (s, o) => {
              if (e$8)
                s.adoptedStyleSheets =
                    o.map((t => t instanceof CSSStyleSheet ? t : t.styleSheet));
              else
                for (const e of o) {
                  const o = document.createElement('style'), n = t$2.litNonce;
                  void 0 !== n && o.setAttribute('nonce', n),
                      o.textContent = e.cssText, s.appendChild(o);
                }
            },
        c$3 = e$8 ? t => t :
   ...
   hasChanged: f$3
    ...
    class b extends HTMLElement {
      ...
      static createProperty(t, s = y$1) {
        if (s.state && (s.attribute = !1), this._$Ei(),
      ...
      static _$Ei() {
        ...
        const t = n$4(this);
        t.finalize(), void 0 !== t.l && (this.l = [...t.l]),
            this.elementProperties = new Map(t.elementProperties);
      }
      static finalize() {
        if (this.hasOwnProperty(d$1('finalized'))) return;
        if (this.finalized = !0, this._$Ei(),
            this.hasOwnProperty(d$1('properties'))) {
          const t = this.properties, s = [...h$3(t), ...o$4(t)];
      ...
      static _$Eu(t, s) {
        ...
        return !1 === i          ? void 0 :
            'string' == typeof i ? i :
            'string' == typeof t ? t.toLowerCase() :
                                   void 0
      }
      constructor() {
        super(), this._$Ep = void 0, this.isUpdatePending = !1,
                 this.hasUpdated = !1, this._$Em = null, this._$Ev();
      }
      _$Ev() {
        this._$Eg = new Promise((t => this.enableUpdating = t)),
        this._$AL = new Map, this._$ES(), this.requestUpdate(),
      ...
      _$EO(t, s) {
        ...
        if (void 0 !== e && !0 === i.reflect) {
          const r = (void 0 !== i.converter?.toAttribute ? i.converter : u$1)
      ...
      performUpdate() {
        ...
        try {
          ...
          t ? (this.willUpdate(s), this._$E_?.forEach((t => t.hostUpdate?.())),
               this.update(s)) :
              this._$Ej();
        } catch (s) {
    ...
    b.elementStyles = [], b.shadowRootOptions = {mode: 'open'},
    b[d$1('elementProperties')] = new Map, b[d$1('finalized')] = new Map,
     ...
     * SPDX-License-Identifier: BSD-3-Clause
     */
    const t$1 = globalThis, i$1 = t$1.trustedTypes,
          s$2 =
              i$1 ? i$1.createPolicy('lit-html', {createHTML: t => t}) : void 0,
          e$6 = '$lit$', h$2 = `lit$${(Math.random() + '').slice(9)}$`,
          o$3 = '?' + h$2, n$3 = `<${o$3}>`, r$4 = document,
          l = () => r$4.createComment(''),
          c$1 = t =>
          ...
          m = RegExp(
              `>|${d}(?:([^\\s"'>=/]+)(${d}*=${
                  d}*(?:[^ \t\n\f\r"'\`<>=]|("|')|))|$)`,
              'g'),
          p = /'/g, g = /"/g, $ = /^(?:script|style|textarea|title)$/i,
          y = t => (i, ...s) => ({_$litType$: t, strings: i, values: s}),
          x = y(1), w = Symbol.for('lit-noChange'),
          T = Symbol.for('lit-nothing'), A = new WeakMap,
      ...
      for (let i = 0; i < s; i++) {
        ...
        for (; y < s.length && (c.lastIndex = y, u = c.exec(s), null !== u);)
          ...
          c === f$2              ? '!--' === u[1] ?
              ...
              c === m            ? '>' === u[0] ? (c = r ?? f$2, d = -1) :
                             void 0 === u[1]    ? d = -2 :
                                                  (d = c.lastIndex - u[2].length, a = u[1],
                                     c = void 0 === u[3] ? m :
                                                       '"' === u[3]    ? g :
                                                                         p) :
              c === g || c === p ? c = m :
        ...
        l += c === f$2 ? s + n$3 :
            d >= 0 ? (o.push(a), s.slice(0, d) + e$6 + s.slice(d) + h$2 + x) :
                     s + h$2 + (-2 === d ? i : x);
      }
    ...
    class V {
      constructor({strings: t, _$litType$: s}, n) {
        ...
        if (this.el = V.createElement(f, n), E.currentNode = this.el.content,
            2 === s) {
          ...
          if (1 === r.nodeType) {
            if (r.hasAttributes())
              for (const t of r.getAttributeNames())
                if (t.endsWith(e$6)) {
                  const i = v[a++], s = r.getAttribute(t).split(h$2),
                        e = /([.?@])?(.*)/.exec(i);
                  d.push({
                    type: 1,
                    index: c,
            ...
            if ($.test(r.tagName)) {
              const t = r.textContent.split(h$2), s = t.length - 1;
              if (s > 0) {
                r.textContent = i$1 ? i$1.emptyScript : '';
                for (let i = 0; i < s; i++)
                  r.append(t[i], l()), E.nextNode(),
            ...
            if (r.data === o$3)
              ...
              for (; -1 !== (t = r.data.indexOf(h$2, t + 1));)
                d.push({type: 7, index: c}), t += h$2.length - 1;
            }
          c++;
        }
      }
      ...
      u(t) {
       ...
       e = (t?.creationScope ?? r$4).importNode(i, !0);
        ...
        for (; void 0 !== l;) {
          if (o === l.index) {
            let i;
            2 === l.type     ? i = new M(h, h.nextSibling, this, t) :
                1 === l.type ? i = new l.ctor(h, l.name, l.strings, this, t) :
    ...
    class M {
      ...
      constructor(t, i, s, e) {
        this.type = 2, this._$AH = T, this._$AN = void 0, this._$AA = t,
      ...
      _(t) {
        this._$AH !== T && c$1(this._$AH) ? this._$AA.nextSibling.data = t :
      ...
      T(t) {
        ...
        for (const h of t)
          e === i.length ?
              i.push(s = new M(this.k(l()), this.k(l()), this, this.options)) :
              s = i[e],
              s._$AI(h), e++;
    ...
    class R {
      ...
      constructor(t, i, s, e, h) {
        this.type = 1, this._$AH = T, this._$AN = void 0, this.element = t,
        this.name = i, this._$AM = e, this.options = h,
        s.length > 2 || '' !== s[0] || '' !== s[1] ?
      ...
      _$AI(t, i = this, s, e) {
        ...
        let o = !1;
        if (void 0 === h)
          t = N(this, t, i, 0), o = !c$1(t) || t !== this._$AH && t !== w,
          o && (this._$AH = t);
        else {
          const e = t;
          let n, r;
          for (t = h[0], n = 0; n < h.length - 1; n++)
            r = N(this, e[s + n], i, n), r === w && (r = this._$AH[n]),
      ...
      O(t) {
        t === T ? this.element.removeAttribute(this.name) :
    ...
    let s$1 = class s extends b{
      constructor() {
        super(...arguments), this.renderOptions = {host: this},
                             this._$Do = void 0;
      } createRenderRoot() {
    ...
    s$1._$litElement$ = !0, s$1[('finalized')] = !0,
...
`;
    ...
    var __decorateClass = (decorators, target, key, kind) => {
      var result = kind > 1 ? void 0 :
          kind              ? __getOwnPropDesc(target, key) :
                              target;
     ...
     */
    const e$5 = (e, t, c) =>
        (c.configurable = !0, c.enumerable = !0,
         Reflect.decorate && 'object' != typeof t &&
    ...
    var ShoelaceElement = class extends s$1 {
      ...
      static define(name, elementConstructor = this, options = {}) {
        ...
        console.warn(`Attempted to register <${name}>${newVersion}, but <${
            name}>${existingVersion} has already been registered.`);
      }
    };
    /* eslint-disable */
    ...
    const oppositeAlignmentMap = {start: 'end', end: 'start'};
    ...
    function getExpandedPlacements(placement) {
      ...
      return [
        getOppositeAlignmentPlacement(placement), oppositePlacement,
        getOppositeAlignmentPlacement(oppositePlacement)
      ];
    }
    function getOppositeAlignmentPlacement(placement) {
      return placement.replace(
          /start|end/g, alignment => oppositeAlignmentMap[alignment]);
    }
    ...
    function getOppositeAxisPlacements(
        placement, flipAlignment, direction, rtl) {
      const alignment = getAlignment(placement);
    ...
    function getOppositePlacement(placement) {
      return placement.replace(
          /left|right|bottom|top/g, side => oppositeSideMap[side]);
    }
     ...
     *
     * This export does not have any `platform` interface logic. You will need
     * to write one for the platform you are using Floating UI with.
     */
    const computePosition$1 = async (reference, floating, config) => {
      ...
      const rtl =
          await (platform.isRTL == null ? void 0 : platform.isRTL(floating));
      let rects =
    ...
    /**
     * Resolves with an object of overflow side offsets that determine how much
     * the element is overflowing a given clipping boundary on each side.
    ...
    async function detectOverflow(state, options) {
      ...
      const clippingClientRect =
          rectToClientRect(await platform.getClippingRect({
            element: ((_await$platform$isEle = await (
                           platform.isElement == null ?
                               void 0 :
                ...
                element.contextElement ||
                    (await (
                        platform.getDocumentElement == null ?
                            void 0 :
      ...
      const rect = elementContext === 'floating' ? {...rects.floating, x, y} :
                                                   rects.reference;
      const offsetParent = await (
          platform.getOffsetParent == null ?
              void 0 :
              platform.getOffsetParent(elements.floating));
      const offsetScale =
          (await (
              platform.isElement == null ? void 0 :
                                           platform.isElement(offsetParent))) ?
          (await (
              platform.getScale == null ? void 0 :
    ...
    /**
     * Provides data to position an inner element of the floating element so
     * that it appears centered to the reference element.
    ...
    const arrow$1 = options => ({
      ...
      async fn(state) {
        ...
        // If the padding is large enough that it causes the arrow to no longer
        // be centered, modify the padding so that it is centered.
        const largestPossiblePadding =
            clientSize / 2 - arrowDimensions[length] / 2 - 1;
        const minPadding = min(paddingObject[minProp], largestPossiblePadding);
        const maxPadding = min(paddingObject[maxProp], largestPossiblePadding);

        // Make sure the arrow doesn't overflow the floating element if the
        // center point is outside the floating element's bounds.
        ...
        // If the reference is small enough that the arrow's padding causes it
        // to to point to nothing for an aligned placement, adjust the offset of
        // the floating element itself. To ensure `shift()` continues to take
        // action, a single reset is performed when this is true.
        const shouldAddOffset = !middlewareData.arrow &&
            getAlignment(placement) != null && center != offset &&
            rects.reference[length] / 2 -
                    (center < min$1 ? minPadding : maxPadding) -
                    arrowDimensions[length] / 2 <
                0;
        const alignmentOffset = shouldAddOffset ?
            center < min$1 ? center - min$1 : center - max :
            0;
        return {
     ...
     * `placement` in order to keep it in view when the preferred placement(s)
     * will overflow the clipping boundary. Alternative to `autoPlacement`.
    ...
    const flip$1 = function(options) {
      ...
      return {
        ...
        async fn(state) {
          ...
          // added, we should skip any logic now since `flip()` has already done
          // its work.
          ...
          if (!overflows.every(side => side <= 0)) {
            ...
            // First, find the candidates that fit on the mainAxis side of
            // overflow, then find the placement that fits the best on the main
            // crossAxis side.
            let resetPlacement =
                (_overflowsData$filter =
                     overflowsData.filter(d => d.overflows[0] <= 0)
    ...
    const offset = function(options) {
      ...
      return {
        ...
        async fn(state) {
          ...
          // If the placement is the same and the arrow caused an alignment
          // offset then we don't need to change the positioning coordinates.
          if (placement ===
    ...
    /**
     * Optimizes the visibility of the floating element by shifting it in order
     * to keep it in view when it will overflow the clipping boundary.
     ...
     * — for instance, prevent it from overflowing the clipping boundary or
      ...
      return {
        ...
        async fn(state) {
          ...
          const nextDimensions =
              await platform.getDimensions(elements.floating);
          if (width !== nextDimensions.width ||
    ...
    function getDocumentElement(node) {
      var _ref;
      return (_ref = (isNode(node) ? node.ownerDocument : node.document) ||
    ...
    function getBoundingClientRect(
        element, includeScale, isFixedStrategy, offsetParent) {
      ...
      const visualOffsets =
          shouldAddVisualOffsets(domElement, isFixedStrategy, offsetParent) ?
    ...
    // Gets the entire size of the scrollable document area, even extending
    // outside of the `<html>` and `<body>` rect bounds if horizontally
    // scrollable.
    function getDocumentRect(element) {
      ...
      const width =
          max(html.scrollWidth, html.clientWidth, body.scrollWidth,
              body.clientWidth);
      const height =
          max(html.scrollHeight, html.clientHeight, body.scrollHeight,
              body.clientHeight);
    ...
    function getInnerBoundingClientRect(element, strategy) {
      const clientRect =
          getBoundingClientRect(element, true, strategy === 'fixed');
      const top = clientRect.top + element.clientTop;
      const left = clientRect.left + element.clientLeft;
      const scale =
          isHTMLElement(element) ? getScale(element) : createCoords(1);
    ...
    function getClientRectFromClippingAncestor(
        element, clippingAncestor, strategy) {
      let rect;
    ...
    function hasFixedPositionAncestor(element, stopNode) {
      ...
      if (parentNode === stopNode || !isElement(parentNode) ||
          isLastTraversableNode(parentNode)) {
        return false;
      }
      return getComputedStyle$1(parentNode).position === 'fixed' ||
    ...
    function getClippingElementAncestors(element, cache) {
      ...
      let result =
          getOverflowAncestors(element, [], false)
    ...
    function getRectRelativeToOffsetParent(element, offsetParent, strategy) {
      ...
      if (isOffsetParentAnElement || !isOffsetParentAnElement && !isFixed) {
        ...
        if (isOffsetParentAnElement) {
          const offsetRect =
              getBoundingClientRect(offsetParent, true, isFixed, offsetParent);
          offsets.x = offsetRect.x + offsetParent.clientLeft;
          offsets.y = offsetRect.y + offsetParent.clientTop;
    ...
    const getElementRects = async function(_ref) {
      ...
      return {
        reference: getRectRelativeToOffsetParent(
            reference, await getOffsetParentFn(floating), strategy),
    ...
    function observeMove(element, onMove) {
      ...
      function refresh(skip, threshold) {
        ...
        const rootMargin = -insetTop + 'px ' + -insetRight + 'px ' +
        ...
        // Older browsers don't support a `document` as the root and will throw
        // an error.
        try {
     ...
     * necessary. Should only be called when the floating element is mounted on
    ...
    function autoUpdate(reference, floating, update, options) {
      ...
      if (elementResize) {
        resizeObserver = new ResizeObserver(_ref => {
          let [firstEntry] = _ref;
          if (firstEntry && firstEntry.target === referenceEl &&
              resizeObserver) {
      ...
      let frameId;
      let prevRefRect =
          animationFrame ? getBoundingClientRect(reference) : null;
      if (animationFrame) {
        frameLoop();
      }
      function frameLoop() {
    ...
    const computePosition = (reference, floating, options) => {
      // This caches the expensive `getClippingElementAncestors` function so
      // that multiple lifecycle resets re-use the same result. It only lives
      // for a single call. If other functions become expensive, we can add them
      // as well.
      const cache = new Map();
    ...
    const t = {
      ...
      ELEMENT: 6
          ...
          e$3 = t => (...e) => ({_$litDirective$: t, values: e});
    class i {
      constructor(t) {}
      get _$AU() {
        return this._$AM._$AU
     ...
     * SPDX-License-Identifier: BSD-3-Clause
     */
    const e$2 = e$3(class extends i {
      constructor(t$1) {
        if (super(t$1),
            t$1.type !== t.ATTRIBUTE || 'class' !== t$1.name ||
      ...
      update(s, [i]) {
        if (void 0 === this.it) {
          ...
          void 0 !== s.strings &&
              (this.st = new Set(
                   s.strings.join(' ').split(/\s/).filter((t => '' !== t))));
        ...
        for (const t in i) {
          const s = !!i[t];
          s === this.it.has(t) || this.st?.has(t) ||
              (s ? (r.add(t), this.it.add(t)) :
    ...
    function offsetParentPolyfill(element) {
      ...
      for (let ancestor = element; ancestor;
      ...
      for (let ancestor = flatTreeParent(element); ancestor;
           ancestor = flatTreeParent(ancestor)) {
        if (!(ancestor instanceof Element)) {
          continue;
        }
      ...
      async handleAnchorChange() {
        ...
        if (this.anchor && typeof this.anchor === 'string') {
          ...
          this.anchorEl = root.getElementById(this.anchor);
            ...
            this.anchor instanceof Element || isVirtualElement(this.anchor)) {
      ...
      reposition() {
          ...
          middleware.push(size({
            apply: ({rects}) => {
              ...
              this.popup.style.width =
                  syncWidth ? `${rects.reference.width}px` : '';
              this.popup.style.height =
                  syncHeight ? `${rects.reference.height}px` : '';
            }
          }));
        } else {
          this.popup.style.width = '';
        ...
        if (this.flip) {
          middleware.push(flip({
            ...
            // @ts-expect-error - We're converting a string attribute to an
            // array here
        ...
        if (this.shift) {
          middleware.push(shift(
              {boundary: this.shiftBoundary, padding: this.shiftPadding}));
        }
        if (this.autoSize) {
          middleware.push(size({
            ...
            apply: ({availableWidth, availableHeight}) => {
              if (this.autoSize === 'vertical' || this.autoSize === 'both') {
                this.style.setProperty(
                    '--auto-size-available-height', `${availableHeight}px`);
              } else {
                this.style.removeProperty('--auto-size-available-height');
              }
              if (this.autoSize === 'horizontal' || this.autoSize === 'both') {
                this.style.setProperty(
                    '--auto-size-available-width', `${availableWidth}px`);
              } else {
                this.style.removeProperty('--auto-size-available-width');
              }
            }
        ...
        computePosition(this.anchorEl, this.popup, {
          ...
          platform:
              __spreadProps(__spreadValues({}, platform), {getOffsetParent})
          ...
          if (this.arrow) {
            ...
            if (this.arrowPlacement === 'start') {
              ...
              right = isRtl ? '' : value;
              left = isRtl ? value : '';
              bottom = typeof arrowY === 'number' ?
                  `calc(${this.arrowPadding}px - var(--arrow-padding-offset))` :
                  '';
            } else if (this.arrowPlacement === 'center') {
              left = typeof arrowX === 'number' ?
    ...
    __decorateClass(
        [n$2({type: Boolean, reflect: true})], SlPopup.prototype, 'active', 2);
    __decorateClass([n$2({reflect: true})], SlPopup.prototype, 'placement', 2);
    __decorateClass([n$2({reflect: true})], SlPopup.prototype, 'strategy', 2);
    __decorateClass([n$2({type: Number})], SlPopup.prototype, 'distance', 2);
    __decorateClass([n$2({type: Number})], SlPopup.prototype, 'skidding', 2);
    __decorateClass([n$2({type: Boolean})], SlPopup.prototype, 'arrow', 2);
    __decorateClass(
        [n$2({attribute: 'arrow-placement'})], SlPopup.prototype,
        'arrowPlacement', 2);
    __decorateClass(
        [n$2({attribute: 'arrow-padding', type: Number})], SlPopup.prototype,
        'arrowPadding', 2);
    ...
    __decorateClass(
        [n$2({attribute: 'flip-fallback-strategy'})], SlPopup.prototype,
        'flipFallbackStrategy', 2);
    __decorateClass(
        [n$2({type: Object})], SlPopup.prototype, 'flipBoundary', 2);
    __decorateClass(
        [n$2({attribute: 'flip-padding', type: Number})], SlPopup.prototype,
        'flipPadding', 2);
    __decorateClass([n$2({type: Boolean})], SlPopup.prototype, 'shift', 2);
    __decorateClass(
        [n$2({type: Object})], SlPopup.prototype, 'shiftBoundary', 2);
    __decorateClass(
        [n$2({attribute: 'shift-padding', type: Number})], SlPopup.prototype,
        'shiftPadding', 2);
    __decorateClass(
        [n$2({attribute: 'auto-size'})], SlPopup.prototype, 'autoSize', 2);
    __decorateClass([n$2()], SlPopup.prototype, 'sync', 2);
    __decorateClass(
        [n$2({type: Object})], SlPopup.prototype, 'autoSizeBoundary', 2);
    __decorateClass(
...
`;
    ...
    function isTakingUpSpace(elem) {
      return Boolean(
          elem.offsetParent || elem.offsetWidth || elem.offsetHeight ||
    ...
    function getTabbableBoundary(root) {
      ...
      const end = (_b = tabbableElements[tabbableElements.length - 1]) != null ?
          _b :
          null;
      return {start, end};
    }
      ...
      function walk(el) {
        if (el instanceof Element) {
          ...
          const slotChildrenOutsideRootElement = (slotElement) => {
            var _a;
            return ((_a = slotElement.getRootNode({composed: true})) == null ?
                        void 0 :
                        _a.host) !== root;
          };
    ...
    function animateTo(el, keyframes, options) {
      return new Promise((resolve) => {
        ...
        const animation =
            el.animate(keyframes, __spreadProps(__spreadValues({}, options), {
      ...
      return Promise.all(el.getAnimations().map((animation) => {
        return new Promise((resolve) => {
          ...
          animation.addEventListener(
              'cancel', () => handleAnimationEvent, {once: true});
          animation.addEventListener(
              'finish', () => handleAnimationEvent, {once: true});
          animation.cancel();
        });
      }));
    }
    ...
    let fallback;
    documentElementObserver.observe(
    ...
    function registerTranslation(...translation) {
      translation.map(t => {
        ...
        if (translations.has(code)) {
          translations.set(
              code,
              Object.assign(Object.assign({}, translations.get(code)), t));
        } else {
    ...
    let LocalizeController$1 = class LocalizeController {
      ...
      getTranslationData(lang) {
        ...
        const language = locale === null || locale === void 0 ?
            void 0 :
            locale.language.toLowerCase();
        const region = (_b = (_a = locale === null || locale === void 0 ?
                                  void 0 :
                                  locale.region) === null ||
                                _a === void 0 ?
                            void 0 :
      ...
      relativeTime(value, unit, options) {
        return new Intl.RelativeTimeFormat(this.lang(), options)
            .format(value, unit);
      }
    };
    ...
    function watch(propertyName, options) {
      const resolvedOptions =
          __spreadValues({waitUntilFirstUpdate: false}, options);
    ...
    var SlDropdown = class extends ShoelaceElement {
        ...
        this.handleDocumentKeyDown = (event) => {
          ...
          if (event.key === 'Tab') {
            ...
            setTimeout(() => {
              var _a2, _b, _c;
              const activeElement =
                  ((_a2 = this.containingElement) == null ? void 0 :
                                                            _a2.getRootNode())
                          instanceof ShadowRoot ?
                  (_c = (_b = document.activeElement) == null ?
                       void 0 :
                       _b.shadowRoot) == null ?
                  void 0 :
                  _c.activeElement :
      ...
      getMenu() {
        return this.panel.assignedElements({flatten: true})
      ...
      //
      // Slotted triggers can be arbitrary content, but we need to link them to
      // the dropdown panel with `aria-haspopup` and `aria-expanded`. These must
      // be applied to the "accessible trigger" (the tabbable portion of the
      // trigger element that gets slotted in) so screen readers will understand
      // them. The accessible trigger could be the slotted element, a child of
      // the slotted element, or an element in the slotted element's shadow
      // root.
      //
      // For example, the accessible trigger of an <sl-button> is a <button>
      // located inside its shadow root.
      //
      // To determine this, we assume the first tabbable element in the trigger
      ...
      /**
       * Instructs the dropdown menu to reposition. Useful when the position or
       * size of the trigger changes when the menu is activated.
       */
      reposition() {
      ...
      async handleOpenChange() {
        ...
        if (this.open) {
          ...
          const {keyframes, options} =
              getAnimation(this, 'dropdown.show', {dir: this.localize.dir()});
          ...
          await stopAnimations(this);
          const {keyframes, options} =
              getAnimation(this, 'dropdown.hide', {dir: this.localize.dir()});
          await animateTo(this.popup.popup, keyframes, options);
      ...
      >
        ...
        <div aria-hidden=${
            this.open ? 'false' : 'true'} aria-labelledby="dropdown">
    ...
    __decorateClass(
        [e$4('.dropdown__trigger')], SlDropdown.prototype, 'trigger', 2);
    __decorateClass(
        [e$4('.dropdown__panel')], SlDropdown.prototype, 'panel', 2);
    __decorateClass(
        [n$2({type: Boolean, reflect: true})], SlDropdown.prototype, 'open', 2);
    __decorateClass(
        [n$2({reflect: true})], SlDropdown.prototype, 'placement', 2);
    __decorateClass(
        [n$2({type: Boolean, reflect: true})], SlDropdown.prototype, 'disabled',
        2);
    __decorateClass(
        [n$2({attribute: 'stay-open-on-select', type: Boolean, reflect: true})],
        SlDropdown.prototype, 'stayOpenOnSelect', 2);
    __decorateClass(
        [n$2({attribute: false})], SlDropdown.prototype, 'containingElement',
        2);
    __decorateClass([n$2({type: Number})], SlDropdown.prototype, 'distance', 2);
    __decorateClass([n$2({type: Number})], SlDropdown.prototype, 'skidding', 2);
    __decorateClass([n$2({type: Boolean})], SlDropdown.prototype, 'hoist', 2);
    __decorateClass(
        [watch('open', {waitUntilFirstUpdate: true})], SlDropdown.prototype,
        'handleOpenChange', 1);
    ...
    setDefaultAnimation('dropdown.hide', {
      keyframes: [{opacity: 1, scale: 1}, {opacity: 0, scale: 0.9}],
      options: {duration: 100, easing: 'ease'}
    });
...
`;
    ...
    var SlMenu = class extends ShoelaceElement {
      ...
      handleKeyDown(event) {
        ...
        } else if (['ArrowDown', 'ArrowUp', 'Home', 'End'].includes(
                       event.key)) {
      ...
      /**
       * @internal Gets all slotted menu items, ignoring dividers, headers, and
       * other elements.
       */
      getAllItems() {
       ...
       * `tabindex="0"` within the roving tab index. The menu item may or may
       * not have focus, but for keyboard interaction purposes it's considered
       * the "active" item.
       */
      getCurrentItem() {
      ...
      /**
       * @internal Sets the current menu item to the specified element. This
       * sets `tabindex="0"` on the target element and `tabindex="-1"` to all
       * other items. This method must be called prior to setting focus on a
       * menu item.
       */
      setCurrentItem(item) {
        const items = this.getAllItems();
...
`;
    ...
    function h$1(i) {
      void 0 !== this._$AN ? (o$1(this), this._$AM = i, r(this)) :
                             this._$AM = i;
    }
    function n$1(i, t = !1, e = 0) {
      ...
      if (void 0 !== h && 0 !== h.size)
        if (t)
          if (Array.isArray(r))
            for (let i = e; i < r.length; i++) s(r[i], !1), o$1(r[i]);
          else
            null != r && (s(r, !1), o$1(r));
        else
          s(this, i);
    ...
    class h {}
    const o = new WeakMap,
          n = e$3(class extends f {
            render(i) {
              return T
            }
            update(i, [s]) {
              const e = s !== this.G;
              return e && void 0 !== this.G && this.ot(void 0),
                     (e || this.rt !== this.lt) &&
                         (this.G = s, this.ct = i.options?.host,
            ...
            ot(t) {
              if ('function' == typeof this.G) {
                ...
                void 0 === s && (s = new WeakMap, o.set(i, s)),
                    void 0 !== s.get(this.G) && this.G.call(this.ct, void 0),
                    s.set(this.G, t), void 0 !== t && this.G.call(this.ct, t);
              } else
                this.G.value = t;
            }
            get rt() {
    ...
    var SubmenuController = class {
      constructor(host, hasSlotController, localize) {
        ...
        this.handleMouseMove = (event) => {
          this.host.style.setProperty(
              '--safe-triangle-cursor-x', `${event.clientX}px`);
          this.host.style.setProperty(
              '--safe-triangle-cursor-y', `${event.clientY}px`);
        };
        ...
        // Prevent the parent menu-item from getting focus on mouse movement on
        // the submenu
        this.handlePopupMouseover = (event) => {
          event.stopPropagation();
        };
        // Set the safe triangle values for the submenu when the position
        // changes
        this.handlePopupReposition = () => {
          const submenuSlot =
              this.host.renderRoot.querySelector('slot[name=\'submenu\']');
          const menu = submenuSlot == null ?
              void 0 :
              submenuSlot.assignedElements({flatten: true})
          ...
          this.host.style.setProperty(
              '--safe-triangle-submenu-start-y', `${top}px`);
          this.host.style.setProperty(
              '--safe-triangle-submenu-end-x',
              `${isRtl ? left + width : left}px`);
          this.host.style.setProperty(
              '--safe-triangle-submenu-end-y', `${top + height}px`);
        };
        ...
        if (!this.isPopupConnected) {
          if (this.popupRef.value) {
            this.popupRef.value.addEventListener(
                'mouseover', this.handlePopupMouseover);
            this.popupRef.value.addEventListener(
                'sl-reposition', this.handlePopupReposition);
      ...
      handleSubmenuEntry(event) {
        ...
        for (const elt of submenuSlot.assignedElements()) {
          menuItems =
              elt.querySelectorAll('sl-menu-item, [role^=\'menuitem\']');
          if (menuItems.length !== 0) {
            break;
          }
        }
      ...
      // Shows the submenu. Supports disabling the opening delay, e.g. for
      // keyboard events that want to set the focus to the newly opened menu.
      ...
      // Calculate the space the top of a menu takes-up, for aligning the popup
      // menu-item with the activating element.
      updateSkidding() {
        var _a;
        if (!((_a = this.host.parentElement) == null ? void 0 :
                                                       _a.computedStyleMap)) {
          return;
        }
        ...
        const skidding = attrs.reduce((accumulator, attr) => {
          var _a2;
          const styleValue = (_a2 = styleMap.get(attr)) != null ?
              _a2 :
              new CSSUnitValue(0, 'px');
          const unitValue = styleValue instanceof CSSUnitValue ?
              styleValue :
              new CSSUnitValue(0, 'px');
    ...
    var HasSlotController = class {
      ...
      test(slotName) {
        return slotName === '[default]' ? this.hasDefaultSlot() :
...
`;
    ...
    function getBasePath(subpath = '') {
      if (!basePath) {
        ...
        const configScript =
            scripts.find((script) => script.hasAttribute('data-shoelace'));
        if (configScript) {
          setBasePath(configScript.getAttribute('data-shoelace'));
        } else {
          const fallbackScript = scripts.find((s) => {
            return /shoelace(\.min)?\.js($|\?)/.test(s.src) ||
  ...
  `
    ...
    var SlIcon = class extends ShoelaceElement {
      ...
      /**
       * Given a URL, this function returns the resulting SVG element or an
      ...
      async resolveIcon(url, library) {
        ...
        try {
          ...
          if (((_a = svg == null ? void 0 : svg.tagName) == null ?
                   void 0 :
                   _a.toLowerCase()) !== 'svg')
      ...
      getIconSource() {
        ...
        if (this.name && library) {
          return {url: library.resolver(this.name), fromLibrary: true};
        }
    ...
    __decorateClass([n$2({reflect: true})], SlIcon.prototype, 'library', 2);
    __decorateClass([watch('label')], SlIcon.prototype, 'handleLabelChange', 1);
    __decorateClass(
        [watch(['name', 'src', 'library'])], SlIcon.prototype, 'setIcon', 1);
    ...
    var SlMenuItem = class extends ShoelaceElement {
      constructor() {
        ...
        this.submenuController =
            new SubmenuController(this, this.hasSlotController, this.localize);
      ...
      /**
       * Returns a text label based on the contents of the menu item's default
       * slot.
       */
      getTextLabel() {
      ...
      >
        ...
        <slot part="label" class="menu-item__label" @slotchange=${
            this.handleDefaultSlotChange}></slot>
        ...
        <span part="submenu-icon" class="menu-item__chevron">
          <sl-icon name=${
            isRtl ?
                'chevron-left' :
                'chevron-right'} library="system" aria-hidden="true"></sl-icon>
        </span>
    ...
    SlMenuItem.dependencies = {'sl-icon': SlIcon, 'sl-popup': SlPopup};
    __decorateClass(
        [e$4('slot:not([name])')], SlMenuItem.prototype, 'defaultSlot', 2);
    __decorateClass([e$4('.menu-item')], SlMenuItem.prototype, 'menuItem', 2);
    __decorateClass([n$2()], SlMenuItem.prototype, 'type', 2);
    __decorateClass(
        [n$2({type: Boolean, reflect: true})], SlMenuItem.prototype, 'checked',
        2);
    __decorateClass([n$2()], SlMenuItem.prototype, 'value', 2);
    __decorateClass(
        [n$2({type: Boolean, reflect: true})], SlMenuItem.prototype, 'disabled',
        2);
    __decorateClass(
        [watch('checked')], SlMenuItem.prototype, 'handleCheckedChange', 1);
    __decorateClass(
        [watch('disabled')], SlMenuItem.prototype, 'handleDisabledChange', 1);
    __decorateClass(
        [watch('type')], SlMenuItem.prototype, 'handleTypeChange', 1);
...
`;
    ...
    __decorateClass(
        [n$2({type: Boolean, reflect: true})], SlDivider.prototype, 'vertical',
        2);
    __decorateClass(
        [watch('vertical')], SlDivider.prototype, 'handleVerticalChange', 1);
    ...
    const faq = [
      {
        ...
        lang: ['zh'],
        args:
            '/home/q&a#%E5%80%8D%E9%80%9F%E5%88%B7%E8%AF%BE%E4%B8%BA%E5%95%A5%E5%AD%A6%E4%B9%A0%E8%BF%9B%E5%BA%A6%E6%B2%A1%E5%8F%98%E5%8C%96'
      },
      {
        title: '百度网盘视频无法倍速播放',
        action: 'openDocsLink',
        lang: ['zh'],
        args:
            '/home/q&a#%E7%99%BE%E5%BA%A6%E7%BD%91%E7%9B%98%E8%A7%86%E9%A2%91%E6%97%A0%E6%B3%95%E5%80%8D%E9%80%9F%E6%92%AD%E6%94%BE'
      },
      {
        ...
        lang: ['zh'],
        args:
            '/home/q&a#%E5%80%8D%E9%80%9F%E6%92%AD%E6%94%BE%E5%8D%A1%E9%A1%BF%E3%80%81%E6%97%A0%E8%BF%9B%E5%BA%A6%E3%80%81%E9%9F%B3%E7%94%BB%E4%B8%8D%E5%90%8C%E6%AD%A5'
      },
      {
        ...
        lang: ['en', 'ru'],
        args:
            '/home/q&a#playback-is-stuttering-has-no-progress-or-is-out-of-sync'
      },
      {
        ...
        lang: ['zh'],
        args:
            '/home/q&a#%E5%A6%82%E4%BD%95%E6%94%AF%E6%8C%81%E6%9C%AC%E5%9C%B0%E8%A7%86%E9%A2%91%E6%96%87%E4%BB%B6%E7%9A%84%E5%80%8D%E9%80%9F%E6%92%AD%E6%94%BE'
      },
      {
      ...
      },
      {
        ...
        lang: ['zh'],
        args:
            '/home/q&a#%E5%BC%80%E5%90%AF%E6%8F%92%E4%BB%B6%E5%90%8E%E7%BD%91%E7%AB%99%E8%87%AA%E8%BA%AB%E7%9A%84%E8%B0%83%E9%80%9F%E5%A4%B1%E6%95%88%E4%BA%86'
      },
      {
        ...
        lang: ['en', 'ru'],
        args:
            '/home/q&a#the-site-s-own-speed-control-fails-after-enabling-the-plugin'
      },
      {
        ...
        lang: ['zh'],
        args:
            '/home/q&a#%E4%B8%BA%E4%BB%80%E4%B9%88%E8%A7%86%E9%A2%91%E8%B7%9F%E9%9F%B3%E9%A2%91%E6%98%AF%E5%88%86%E5%BC%80%E4%B8%8B%E8%BD%BD%E7%9A%84'
      },
      {
      ...
      },
      {
        ...
        lang: ['zh'],
        args:
            '/home/q&a#%E5%A6%82%E4%BD%95%E5%90%88%E5%B9%B6%E4%B8%8B%E8%BD%BD%E5%88%B0%E7%9A%84%E9%9F%B3%E8%A7%86%E9%A2%91%E6%96%87%E4%BB%B6'
      },
      {
      ...
      },
      {
        ...
        lang: ['zh'],
        args:
            '/home/q&a#%E4%B8%BA%E4%BB%80%E4%B9%88%E8%A7%86%E9%A2%91%E6%88%AA%E5%9B%BE%E4%B8%8D%E5%90%8C%E7%BD%91%E7%AB%99%E8%A1%A8%E7%8E%B0%E4%B8%8D%E4%B8%80%E8%87%B4'
      },
      {
        title:
        ...
        lang: ['en', 'ru'],
        args:
            '/home/q&a#why-is-video-screenshot-functionality-inconsistent-across-websites'
      },
      {
        title: '如何禁用或自定义快捷键',
        action: 'openDocsLink',
        lang: ['zh'],
        args:
            '/home/q&a#%E5%A6%82%E4%BD%95%E7%A6%81%E7%94%A8%E6%88%96%E8%87%AA%E5%AE%9A%E4%B9%89%E5%BF%AB%E6%8D%B7%E9%94%AE'
      },
      {
    ...
    const menuConfig = [
      ...
      },
      {
        ...
        icon: img$3,
        dropdownMenu: [
          {title: '0.5x', desc: '0.5x', action: 'setPlaybackRate', args: 0.5}, {
            title: '0.75x',
            desc: '0.75x',
            action: 'setPlaybackRate',
            args: 0.75
          },
          {title: '1.0x', desc: '1.0x', action: 'setPlaybackRate', args: 1}, {
            title: '1.25x',
            desc: '1.25x',
            action: 'setPlaybackRate',
            args: 1.25
          },
          {title: '1.5x', desc: '1.5x', action: 'setPlaybackRate', args: 1.5},
          {title: '2.0x', desc: '2.0x', action: 'setPlaybackRate', args: 2},
          {title: '3.0x', desc: '3.0x', action: 'setPlaybackRate', args: 3},
          {title: '4.0x', desc: '4.0x', action: 'setPlaybackRate', args: 4},
          {title: '5.0x', desc: '5.0x', action: 'setPlaybackRate', args: 5},
          {title: '6.0x', desc: '6.0x', action: 'setPlaybackRate', args: 6},
          {title: '7.0x', desc: '7.0x', action: 'setPlaybackRate', args: 7},
          {title: '8.0x', desc: '8.0x', action: 'setPlaybackRate', args: 8},
          {title: '16.0x', desc: '16.0x', action: 'setPlaybackRate', args: 16},
          {title: '32.0x', desc: '32.0x', action: 'setPlaybackRate', args: 32},
          {title: '64.0x', desc: '64.0x', action: 'setPlaybackRate', args: 64}
        ]
      },
      {
        title: i18n.t('menu'),
        ...
        dropdownMenu: [
          ...
          },
          {
            ...
            subMenu: [
              ...
              {title: i18n.t('comingSoon'), desc: i18n.t('comingSoon')}
            ]
          },
          {divider: true}, {
          ...
          {title: i18n.t('faq'), desc: i18n.t('faq'), subMenu: faq}, {
    ...
    function convertDropdownMenuToTemplate(dropdownMenu, isRootMenu = true) {
      const menuItems =
          dropdownMenu
              .map(item => {
                ...
                if (lang && !i18n.isMatchCurLang(lang)) return ''
                  ...
                  const title = (item.title instanceof Function ? item.title() :
                                                                  item.title) ||
                      '';
                const desc =
        ...
        <sl-menu-item class="h5p-menu-action" value="${id}" title="${
                      desc ||
                      title}" data-action="${item.action || ''}" data-args='${
      ...
      `
                  ...
                  return `<sl-menu-item class="h5p-menu-action" value="${
                      id}" title="${desc || title}" data-action="${
                      item.action || ''}" data-args='${
                      JSON.stringify(
                          item.args || null)}' data-url="${item.url || ''}">
        ${title}
      </sl-menu-item>
      `
                }
          ...
          menuConfig
              .map(item => {
                ...
                const desc =
                    ...
                    '';
                const iconHtml =
                    item.icon ? `<sl-icon src="${item.icon}"></sl-icon>` : '';
                const menuDesc = item.iconOnly && iconHtml ?
                    '' :
            ...
            <sl-dropdown distance="6">
              <span slot="trigger" class="h5p-action-btn" title="${
                      desc || title}" data-title="${title}" data-action="${
                      item.action || ''}">
                ${iconHtml}
                ${menuDesc}
              </span>
  ...
  `
    ...
    function createLogoModTemplate() {
      ...
      return `<a class="h5p-logo-mod" href="${
          homepage}" target="_blank">h5player</a>`
    }
    ...
    const defaultRecommendList = [
      {
        ...
        priority: 99,
        i18n: {en: {title: '【h5player】User Manual'}}
      },
      {
        title: '【h5player】音视频一键合并工具，无需二次编码',
        desc:
            '将h5player下载到的音视频文件自动合并成一个文件，不经过二次编码，可快速合并',
        url: 'https://u.anzz.top/ffmpegscript',
        i18n: {
          en: {
            title:
                '【h5player】Audio and video merge tool, no secondary coding required',
            desc:
                'Automatically merge the audio and video files downloaded by h5player into one file without secondary coding, which can be quickly merged'
          }
        }
      },
      {
        ...
        i18n: {
          en: {
            title:
                '【Hello-AI】It\'s not AI that takes away your job, but the person who knows how to use AI tools'
          }
        }
      }
    ];
    ...
    function createRecommendModTemplate(refDom) {
      const showMod = isGlobalStorageUsable &&
      ...
      if (recommendList.length > 5) {
        recommendList =
            recommendList.sort(() => Math.random() - 0.5).slice(0, 5);
      }

      /* 根据recommendList里的priority字段进行排序，priority值越大越靠前 */
      recommendList =
          recommendList.sort((a, b) => (b.priority || 0) - (a.priority || 0));
      ...
      const recommendHtml =
          recommendList
              .map(item => {
                ...
                if (item.i18n) {
                  const i18nInfo = item.i18n[`${curLang}`] ||
                      item.i18n[`${curLang2}`] || item.i18n[`${curLang3}`] ||
                      item.i18n[`${curLang4}`];
                  if (i18nInfo) {
                ...
                return `<a class="h5p-recommend-item" href="${url}" title="${
    ...
    function registerRecommendModToggle(recommendWrap, reRender) {
      ...
      const toggleRecommend = () => {
        ...
        const recommendItems =
            recommendWrap.querySelectorAll('.h5p-recommend-item');
      ...
      recommendWrap.__h5pRecommendModInterval__ =
          setInterval(toggleRecommend, 3000);
      if (!reRender) {
    ...
    function menuActionHandler(obj) {
      const {event, h5Player, h5playerUI, videoElement, popup, actionCallback} =
          obj;
      const target = event.target;
      ...
      if (action &&
          (h5Player[action] instanceof Function || globalFunctional[action])) {
    ...
    const h5playerUI = {
      ...
      popup(element, h5Player) {
        if (this.__disableGUITemporarily__ ||
        ...
        /* 防止popup死循环渲染 */
        if (element.__popupRenderedCount__ &&
            element.__popupRenderedCount__ > 15) {
          return false
        } else {
          element.__popupRenderedCount__ = element.__popupRenderedCount__ ?
        ...
        if (!popupWrapId) {
          popupWrapId =
              'h5player-popup-wrap-' + Math.random().toString(36).substr(2);
        ...
        const menuTemplate =
            createMenuTemplate(menuConfigPreprocess(menuConfig, element));
        popupWrap = parseHTML(
            `
        ...
        <div class="h5player-popup-content">
          ...
          <div class="h5p-recommend-wrap">
            <div style="overflow:hidden">${
                createRecommendModTemplate(element)}</div>
          </div>
    ...
    `,
        ...
        function checkPopupUpdateComplete() {
          if (!popup || !popup.updateComplete || !popup.updateComplete.then) {
            // debug.error('[h5playerUI][popup][updateComplete],
            // 组件初始化异常', popup, element)
        ...
        function reRenderMenuMod() {
          const menuWrap =
              popupWrap.querySelector('.h5player-popup-content .h5p-menu-wrap');
        ...
        function reRenderRecommendMod() {
          ...
          if (recommendWrap && recommendMod) {
            ...
            const newRecommendModTemplate = `<div style="overflow:hidden">${
                createRecommendModTemplate(element)}</div>`;
        ...
        popupWrap.addEventListener('mouseleave', () => {
          ...
          if (isOutOfDocument(element)) {
            ...
            mouseleaveTimer = setTimeout(() => {
              !alwaysShowUIBar && !element.paused &&
        ...
        async function openDropdown(event) {
          ...
          if (!(target.classList.contains(actionBtnClass) ||
                target.parentElement.classList.contains(actionBtnClass))) {
            return false
          }

          const dropdowns = popupWrap.querySelectorAll('sl-dropdown');
          const curDropdown =
              target.parentElement.tagName.toLowerCase() === 'sl-dropdown' ?
              target.parentElement :
              target.parentElement.parentElement;
          const isDropdownDom = curDropdown &&
...
const h5Player = {
  ...
  defaultTransform:
      {scale: 1, translate: {x: 0, y: 0}, rotate: 0, rotateY: 0, rotateX: 0},
  ...
  toggleHotkeys() {
    const confirm = window.confirm(
        this.__disableHotkeysTemporarily__ ? i18n.t('enableHotkeys') :
  ...
  getPlayerWrapDom: function() {
    ...
    eachParentNode(player, function(parent) {
      if (parent === document || !parent.getBoundingClientRect)
        return const parentBox = parent.getBoundingClientRect();
      if (parentBox.width && parentBox.height) {
        if (parentBox.width === playerBox.width &&
  ...
  initPlayerInstance(isSingle) {
    ...
    needInitEvent && player.addEventListener('enterpictureinpicture', () => {
      monkeyMsg.send('globalPictureInPictureInfo', {usePictureInPicture: true});
    ...
    needInitEvent && player.addEventListener('leavepictureinpicture', () => {
      ...
      monkeyMsg.send(
          'globalPictureInPictureInfo', {usePictureInPicture: false});
    ...
    function updataBufferedTime(player) {
      ...
      if (t.autoGotoBufferedTime && player.bufferedTime &&
          t.player() === player && player.bufferedTime < player.duration - 1 &&
    ...
    needInitEvent && player.addEventListener('loadeddata', function() {
      debug.log(
          `[player][loadeddata] ${player.src} video duration: ${
              player.duration} video dom:`,
          player);
      srcRecord(player);
    });
    ...
    needInitEvent && player.addEventListener('timeupdate', function() {
      ...
      if (Date.now() - lastRegisterUIPopupTime > 800 &&
  ...
  isLeavepictureinpictureAwhile() {
    const t = this;
    return t.leavepictureinpictureTime &&
  ...
  proxyPlayerInstance(player) {
    ...
    if (!player._hangUp_) {
       ...
       *     player方法或属性名，名字写对外，还须要该方法或属性被代理了才能进行挂起，否则这将是个无效的调用
  ...
  initAutoPlay: function(p) {
    ...
    if (taskConf.autoPlay) {
      ...
      addMenu({
        title: () => configManager.getLocalStorage('media.autoPlay') ?
    ...
    if (!configManager.get('media.autoPlay') || (!p && t.hasInitAutoPlay) ||
        !player || (p && p !== t.player()) || document.hidden) {
      return false
    }
  ...
  setPlaybackRate: function(num, notips, duplicate, skipLock) {
    ...
    try {
      ...
      originalMethods.Object.defineProperty.call(
          Object, player, 'playbackRate', {
            configurable: true,
            get: function() {
              /**
               * 在油管，如果返回的是playbackRateDescriptor.get.apply(player,
               * arguments)，调速会出现波动和异常
  ...
  setPlaybackRatePlus: function(num) {
    ...
    t.playbackRatePlusInfo[num] =
        t.playbackRatePlusInfo[num] || {time: Date.now() - 1000, value: num};
  ...
  isLockCurrentTime: function() {
    ...
    const player = this.player();
    if (player && player.currentTimeInfo &&
  ...
  setCurrentTime: function(num) {
    if (!num && num !== 0) return num = Number(num);
  ...
  setCurrentTimeDown(num) {
    ...
    if (TCC.doTask('subtractCurrentTime'))
      ...
      if (this.player()) {
        ...
        this.tips(
            i18n.t('tipsMsg.backward') + Math.abs(num) +
  ...
  setTransform(notTips) {
    ...
    const mirror = t.rotateX === 180 ?
        `rotateX(${t.rotateX}deg)` :
        (t.rotateY === 180 ? `rotateY(${t.rotateY}deg)` : '');
    player.style.transform = `scale(${scale}) translate(${translate.x}px, ${
        translate.y}px) rotate(${t.rotate}deg) ${mirror}`;
  ...
  /**
   * 定格帧画面
   * @param perFps {Number} -可选 默认
   *     1，即定格到下一帧，如果是-1则为定格到上一帧
   */
  freezeFrame(perFps) {
    perFps = perFps || 1;
    const t = this;
  ...
  toggleAutoGotoBufferedTime() {
    ...
    t.tips(
        t.autoGotoBufferedTime ? i18n.t('autoGotoBufferedTime') :
  ...
  tips: function(str) {
    ...
    if (!isAudio) {
      ...
      const oldPosition = parentNode.getAttribute('def-position') ||
      ...
      if (Math.abs(newPlayerBox.height - playerBox.height) > 50) {
        ...
        // debug.info('应用新样式后给播放器高宽造成了严重的偏差，样式已被还原：',
        // player, playerBox, newPlayerBox)
      }
    }
  ...
  initTips: function() {
    ...
    const isAudio = t.isAudioInstance();
    const parentNode = isAudio ? document.body : t.getTipsContainer();
    if (parentNode.querySelector('.' + t.tipsClassName))
      return
  ...
  setFilter(item, num, isDown) {
    ...
    const nameMap =
        {0: 'brightness', 1: 'contrast', 2: 'saturation', 3: 'hue', 4: 'blur'};
  ...
  palyerTrigger: function(player, event) {
    if (!player || !event) return const t = h5Player;
    const keyCode = event.keyCode;
    const key = event.key.toLowerCase();
    ...
    if (event.shiftKey && !event.ctrlKey && !event.altKey && !event.metaKey) {
      ...
      // 视频画面缩放相关事件
      const allowKeys =
          ['x', 'c', 'z', 'arrowright', 'arrowleft', 'arrowup', 'arrowdown'];
  ...
  runCustomShortcuts: function(player, event) {
    if (!player || !event) return const key = event.key.toLowerCase();
    const taskConf = TCC.getTaskConfig();
    ...
    if (confIsCorrect && isRegister()) {
      // 执行自定义快捷键操作
      const isDo = TCC.doTask('shortcuts', {event, player, h5Player});
  ...
  keydownEvent: function(event) {
    ...
    const target = event.composedPath ?
        event.composedPath()[0] || event.target :
        event.target;
    if (t.__disableHotkeysTemporarily__ || isEditableTarget(target))
      return
    ...
    if (!player) {
      if (t.hasCrossOriginVideoDetected) {
        ...
        if (t.hotkeysRunner && t.hotkeysRunner.run) {
          t.hotkeysRunner.run(
              {event, stopPropagation: true, preventDefault: true});
        } else {
          t.registerHotkeysRunner();
          event.stopPropagation();
          event.preventDefault();
  ...
  /**
   * 获取播放进度
   * @param player -可选 对应的h5 播放器对象，
   *     如果不传，则获取到的是整个播放进度表，传则获取当前播放器的播放进度
   */
  getPlayProgress: function(player) {
    ...
    if (!player) {
      ...
      if (progressMap[keyName]) {
         ...
         */
        if (Number.isNaN(Number(player.duration)) ||
  ...
  playProgressRecorder: function(player) {
    ...
    function recorder(player) {
      player._playProgressTimer_ = setTimeout(function() {
        /* 时长小于两分钟的视频不记录播放进度 */
        const isToShort = !player.duration ||
            Number.isNaN(Number(player.duration)) || player.duration < 120;
        ...
        if (list.length > 10) {
          ...
          list.forEach(function(keyName) {
            progressMap[keyName] && progressMap[keyName].t &&
  ...
  setPlayProgress: function(player) {
     ...
     */
    if (!curTime || Number.isNaN(curTime) || curTime < 10 ||
  ...
  setPlayerInstance(el) {
    ...
    if (isVideoElement(el)) {
      ...
      if (elInfo && elInfo.width > 200 && parentElInfo &&
          parentElInfo.width > 200) {
        t.playerInstance = el;
        t.initPlayerInstance(false);
      }
    } else if (isAudioElement(el)) {
      if (isAudioElement(t.playerInstance) ||
  ...
  detecH5Player: function() {
    ...
    if (playerList.length) {
      ...
      if (isInCrossOriginFrame()) {
        /* 广播检测到H5Player的消息 */
        monkeyMsg.send('videoDetected', {src: t.playerInstance.src});
      }
  ...
  bindFakeEvent() {
     ...
     */
    if (!crossTabCtl.hasOpenPictureInPicture() &&
  ...
  setCustomConfiguration(config, tag = 'Default') {
    if (!config) return false
      ...
      const configuration =
          configManager.mergeDefConf(config.customConfiguration);
  ...
  init: function(global) {
    ...
    if (TCC && TCC.doTask('disable') === true) {
      debug.info(`[TCC][disable][${
          location
              .host}] 已禁止在该网站运行视频检测逻辑，您可查看任务配置中心的相关配置了解详情`);
      return true
    }
    ...
    monkeyMsg.on('videoDetected', async (name, oldVal, newVal, remote) => {
      ...
      debug.log(
          '[hasCrossOriginVideoDetected]', t, name, oldVal, newVal, remote);
    });
...
async function h5PlayerInit() {
  ...
  const blackDomainList = configManager.get('blacklist.domains') || [];
  const isInBlackList = blackUrlList.includes(location.href) ||
      blackDomainList.includes(location.host);

  if (isInBlackList) {
    console.warn(`[h5player][config][blacklist][${
        location
  ...
  try {
    if (isEnabled && !isInBlackList) {
      ...
      if (configManager.get('enhance.allowExperimentFeatures') &&
        ...
        mediaSource.init();
        debug.warn(`[experimentFeatures][warning] ${
            i18n.t('experimentFeaturesWarning')}`);
  ...
  if (!isEnabled || isInBlackList) {
    debug.warn(`[config][disable][${
        location.host}] 当前网站已禁用脚本，如要启用脚本，请在菜单里开启`);
    return false
  }
  ...
  if (!window.document || !window.document.documentElement) {
    setTimeout(() => {
      if (retryCount < 200) {
        ...
        console.error(
            '[h5player message:]', 'not documentElement detected!', window);
      }
    }, 10);
    ...
    console.warn(
        '[h5player message:]', 'documentElement detected!', retryCount, window);
  }
