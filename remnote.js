...
(function () {
  ...
  // Helper to handle Ctrl/Cmd+E while a textarea is focused.
  // Responsibilities:
  //  - blur the provided textarea
  //  - prevent default and stop propagation on the original event
  //  - after a short delay, synthesize keydown/keypress/keyup and an input
  //    event for the character 'e' targeting the currently active element
  function handleCtrlEInTextarea(textarea, event) {
    log(LOG_LEVELS.INFO, '[Ctrl+E] Detected in textarea — handling via helper');
    try {
      textarea.blur();
    } catch (err) {
      log(LOG_LEVELS.WARN, '[Ctrl+E] Failed to blur textarea:', err);
    }
    log(LOG_LEVELS.DEBUG, '[Ctrl+E] textarea blurred; waiting 50ms before dispatching synthetic "e" events');
    ...
    try {
      ...
      // Some synthetic events may not allow this; log and continue
      log(LOG_LEVELS.WARN, '[Ctrl+E] Failed to preventDefault/stopPropagation:', err);
    }

    setTimeout(() => {
      log(LOG_LEVELS.DEBUG, '[Ctrl+E] Dispatching synthetic key events for "e"');

      const target = document.activeElement || document.body;
      log(LOG_LEVELS.DEBUG, '[Ctrl+E] Synthetic events target:', target);

      const makeKeyboardEvent = (type) => new KeyboardEvent(type, {
        key: 'e',
        code: 'KeyE',
        keyCode: 69,
        which: 69,
      ...
      ['keydown', 'keypress', 'keyup'].forEach(type => {
        const ev = makeKeyboardEvent(type);
        log(LOG_LEVELS.DEBUG, `[Ctrl+E] dispatching ${type} to`, target);
        target.dispatchEvent(ev);
      });

      // Also dispatch an input event in case consumers watch for it
      try {
        ...
        log(LOG_LEVELS.DEBUG, '[Ctrl+E] dispatching input event to', target);
        target.dispatchEvent(inputEv);
      } catch (err) {
        log(LOG_LEVELS.WARN, '[Ctrl+E] InputEvent dispatch failed:', err);
      }

      log(LOG_LEVELS.INFO, '[Ctrl+E] Synthetic "e" events dispatched');
    }, 50);
  }
    ...
    function checkAndHighlightRemnoteElements() {
      try {
        ...
        if (highlightedCount > 0) {
          log(LOG_LEVELS.INFO,
            `[initRemnoteFlashcards] Highlighted ${highlightedCount} new Remnote elements`);
        }
      } catch (error) {
        log(LOG_LEVELS.INFO,
  ...
  function promptForHotkey() {
    ...
    const content = `
              <h3 style="margin-top: 0; color: var(--text-color, #e0e0e0);">Script Configuration for ${domain}</h3>
              ...
              <div style="margin-bottom: 15px;">
                  <label style="color: var(--text-color, #e0e0e0);"><input type="checkbox" id="hotkeyCtrl" ${hotkey.ctrlKey ? 'checked' : ''}> ⌃(Ctrl)</label>
                  <label style="margin-left: 10px; color: var(--text-color, #e0e0e0);"><input type="checkbox" id="hotkeyShift" ${hotkey.shiftKey ? 'checked' : ''}> ⇧(Shift)</label>
                  <label style="margin-left: 10px; color: var(--text-color, #e0e0e0);"><input type="checkbox" id="hotkeyAlt" ${hotkey.altKey ? 'checked' : ''}> ⌥(Alt)</label>
                  <label style="margin-left: 10px; color: var(--text-color, #e0e0e0);"><input type="checkbox" id="hotkeyMeta" ${hotkey.metaKey ? 'checked' : ''}> ⌘(Cmd)</label>
              </div>
              <div style="margin-bottom: 15px;">
                  ...
                  <select id="logLevel" style="background: var(--input-background, #3d3d3d); color: var(--text-color, #e0e0e0); border: 1px solid var(--border-color, #555); padding: 5px;">
                      <option value="${LOG_LEVELS.ERROR}" ${currentLogLevel === LOG_LEVELS.ERROR ? 'selected' :
        ''}>Error Only</option>
                      <option value="${LOG_LEVELS.WARN}" ${currentLogLevel === LOG_LEVELS.WARN ? 'selected' :
        ''}>Warning & Error</option>
                      <option value="${LOG_LEVELS.INFO}" ${currentLogLevel === LOG_LEVELS.INFO ? 'selected' :
        ''}>Info & Above</option>
                      <option value="${LOG_LEVELS.DEBUG}" ${currentLogLevel === LOG_LEVELS.DEBUG ? 'selected' :
        ''}>Debug (All)</option>
                  </select>
              </div>
  ...
  function setupCtrlIForIgnoreButton() {

    let isListenerAdded = false;

    function tryAddListener() {
      const ignoreButton = document.querySelector('button.rn-button:nth-of-type(2)');
      if (ignoreButton && ignoreButton.textContent.trim() === 'Ignore') {
        log(LOG_LEVELS.INFO, '[Ctrl+I] Ignore button found and validated.');
        if (!isListenerAdded) {
          document.addEventListener('keydown', (event) => {
            if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 'i') {
              event.preventDefault();
              ignoreButton.click();
              log(LOG_LEVELS.INFO, '[Ctrl+I] Ignore button clicked.');
            }
          });
          isListenerAdded = true;
        }
      } else {
        log(LOG_LEVELS.DEBUG, '[Ctrl+I] Ignore button not found or text does not match.');
      }
    }

    // Initial check and periodic observer for dynamic content
    tryAddListener();
    const observer = new MutationObserver(() => tryAddListener());
    observer.observe(document.body, { childList: true, subtree: true });
  }

  setupCtrlIForIgnoreButton();
